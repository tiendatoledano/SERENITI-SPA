...
async function bloquearIP(ip, motivo, tipoAtaque, duracionMs, ruta) {
    try {
        ...
        const existe = bloqueos.some(b => b.ip === ip && !b.expirado);
        if (existe) {
            return false;
        }
        
        const ahora = Date.now();
        const expiracion = duracionMs ? ahora + duracionMs : ahora + 3600000; // 1 hora por defecto
        
        const nuevoBloqueo = {
            id: Date.now().toString(36) + Math.random().toString(36).substr(2, 4),
            ip: ip,
            motivo: motivo || 'Comportamiento sospechoso',
            tipoAtaque: tipoAtaque || 'DESCONOCIDO',
        ...
        await saveJSON(BLOQUEOS_FILE, bloqueos);
        console.log(`🛡️ IP ${ip} bloqueada: ${motivo}`);
        return true;
    } catch (error) {
    ...
    try {
        ...
        const actualizados = bloqueos.map(b => {
            if (b.ip === ip && !b.expirado) {
                encontrado = true;
                return { ...b, expirado: true, expiracion: Date.now() - 1000 };
            }
            return b;
        });
...
async function bloquearIPAdmin(req, res) {
    try {
        ...
        const duracionMs = (duracion || 60) * 60 * 1000; // minutos a ms
