...
async function seguridadMiddleware(req, res, next) {
    ...
    const excludedPaths = ['/bloqueado', '/api/seguridad', '/api/verify', '/api/login', '/api/config-emailjs', '/auth/magic'];
...
// ============================================================
// ⭐ NUEVA FUNCIÓN: LIMPIAR BLOQUEOS EXPIRADOS
// ============================================================
...
async function limpiarBloqueosExpirados() {
    try {
        ...
        // Filtrar bloqueos que no han expirado
        const activos = bloqueos.filter(b => {
            // Si ya está marcado como expirado, eliminarlo
            if (b.expirado === true) return false;
            // Si la expiración es menor que ahora, ya expiró
            if (b.expiracion && b.expiracion < ahora) return false;
            return true;
        ...
        await saveJSON(BLOQUEOS_FILE, activos);
        const eliminados = bloqueos.length - activos.length;
        console.log(`🧹 ${eliminados} bloqueos expirados eliminados`);
        return { eliminados, activos: activos.length };
    } catch (error) {
        console.error('❌ Error limpiando bloqueos expirados:', error);
        return { eliminados: 0, activos: 0 };
    }
}

// ============================================================
// ⭐ NUEVA FUNCIÓN: ELIMINAR TODOS LOS BLOQUEOS
// ============================================================

async function eliminarTodosLosBloqueos() {
    try {
        await saveJSON(BLOQUEOS_FILE, []);
        console.log('🧹 Todos los bloqueos eliminados');
        return { success: true };
    } catch (error) {
        console.error('❌ Error eliminando todos los bloqueos:', error);
