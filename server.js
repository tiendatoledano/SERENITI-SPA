...
// ============================================================
// Almacenar tokens de admin en memoria
...
app.get('/api/testimonios', async (req, res) => {
    try {
        ...
        console.log(`✅ Testimonios públicos: ${publicos.length}`);
        res.json(publicos);
...
// ============================================================
// RUTAS DE ADMIN PARA TESTIMONIOS (PROTEGIDAS)
// ============================================================

// GET - Obtener todos los testimonios (para admin)
...
app.post('/turnos', async (req, res) => {
    try {
        ...
        if (!nombre || !telefono || !massageType || !dia || hora === undefined || hora === null) {
            console.error('❌ Faltan campos obligatorios:', { nombre, telefono, massageType, dia, hora });
            return res.status(400).json({ error: 'Faltan campos obligatorios: nombre, teléfono, masaje, día y hora' });
        }
        
        // Validar que la hora sea un número
        const horaNumerica = parseInt(hora);
        if (isNaN(horaNumerica) || horaNumerica < 0 || horaNumerica > 23) {
            console.error('❌ Hora inválida:', hora);
            return res.status(400).json({ error: 'Hora inválida. Debe ser un número entre 0 y 23.' });
        }
        ...
        try {
            ...
            console.log('📝 Creando nuevo archivo de turnos');
            turnos = [];
        }
        ...
        if (ocupado) {
            console.warn('⚠️ Horario ocupado:', { dia: diaLower, hora: horaNumerica, fecha: fechaTurno });
            return res.status(409).json({ error: 'Horario ocupado. Selecciona otro horario.' });
        }
...
// ============================================================
// RUTAS PARA ADMIN - GESTIÓN DE USUARIOS (PROTEGIDAS)
// ============================================================
