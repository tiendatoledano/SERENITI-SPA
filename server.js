...
// ============================================================
// DECLARAR VARIABLE GLOBAL DE SUPABASE
// ============================================================
let supabase = null;
const SUPABASE_TABLE = 'registro_config';

// ============================================================
// LOGS DETALLADOS - COLORES PARA CONSOLA
// ============================================================
const colors = {
    reset: '\x1b[0m',
    bright: '\x1b[1m',
...
function logInfo(msg, data = null) {
    ...
    if (data) console.log(`${colors.blue}   └─ ${JSON.stringify(data, null, 2)}${colors.reset}`);
}

function logSuccess(msg, data = null) {
    console.log(`${colors.green}✅ [SUCCESS]${colors.reset} ${msg}`);
    if (data) console.log(`${colors.green}   └─ ${JSON.stringify(data, null, 2)}${colors.reset}`);
}
...
function logError(msg, error = null) {
    ...
    if (error) {
        ...
        if (error.stack) console.log(`${colors.red}   └─ Stack: ${error.stack}${colors.reset}`);
    }
}

function logWarn(msg, data = null) {
    console.log(`${colors.yellow}⚠️ [WARNING]${colors.reset} ${msg}`);
    if (data) console.log(`${colors.yellow}   └─ ${JSON.stringify(data, null, 2)}${colors.reset}`);
}

function logRequest(req, extra = '') {
    console.log(`${colors.magenta}📨 [REQUEST]${colors.reset} ${req.method} ${req.path} ${extra}`);
}
...
try {
    ...
    if (SUPABASE_URL && SUPABASE_ANON_KEY) {
        ...
        logSuccess('✅ Supabase inicializado correctamente');
    } else {
        logWarn('⚠️ Supabase no configurado. Usando almacenamiento local.');
    }
} catch (e) {
    logWarn('⚠️ Módulo @supabase/supabase-js no encontrado. Usando almacenamiento local.', e);
}
...
if (!fsSync.existsSync(DATA_DIR)) {
    ...
    logInfo(`Directorio de datos creado: ${DATA_DIR}`);
}
...
app.use((req, res, next) => {
    ...
    res.send = function(data) {
        ...
        console.log(`${colors.magenta}📤 [RESPONSE]${colors.reset} ${req.method} ${req.path} ${statusColor}${res.statusCode}${colors.reset} ${duration}ms`);
        originalSend.call(this, data);
    };
    
    logRequest(req, `(IP: ${ip})`);
    next();
});
...
// ============================================================
// PALABRAS PROHIBIDAS Y SISTEMA DE BLOQUEOS
// ============================================================
...
async function cargarBloqueosHistoricos() {
    try {
        ...
        logError('Error cargando bloqueos históricos:', e);
    }
}
...
function bloquearIPConAcumulativo(ip, motivo, tipo, palabraOfensiva, permanente = false) {
    ...
    logWarn(`🔴 IP BLOQUEADA: ${ipLimpia} - ${motivo} (${conteo}ra vez)`);
    return ipLimpia;
}
...
async function initAllFiles() {
    logInfo('🔧 Inicializando archivos en:', DATA_DIR);
    ...
    if (!fsSync.existsSync(CONFIG_FILE)) {
        ...
        logSuccess('✅ config.json creado');
    } else {
        try {
            const data = JSON.parse(await fs.readFile(CONFIG_FILE, 'utf8'));
            configData = data;
            logSuccess('✅ config.json cargado correctamente');
        } catch(e) {
            logError('Error cargando config.json:', e);
        }
    }
    ...
    if (!fsSync.existsSync(PALABRAS_BANEADAS_FILE)) {
        ...
        logSuccess('✅ palabras-baneadas.json creado');
    }
    ...
    if (!fsSync.existsSync(REGISTRO_FILE)) {
        ...
        logSuccess('✅ registro-config.json creado');
    }
    ...
    if (fsSync.existsSync(BLOQUEOS_FILE)) {
        try {
            ...
            logError('Error cargando bloqueos:', e);
        }
    }
    ...
    try {
        ...
        logSuccess(`✅ Servicios cargados: ${serviciosData.length}`);
    } catch(e) {
        logError('Error cargando servicios:', e);
        serviciosData = [];
    }
    ...
    try {
        ...
        logSuccess(`✅ Ubicación del salón: ${ubicacionSalonGlobal}`);
    } catch(e) {
        logWarn('Error cargando ubicación del salón:', e);
    }
    
    try {
        const data = JSON.parse(await fs.readFile(PALABRAS_BANEADAS_FILE, 'utf8'));
        palabrasBaneadas = data.palabras || [];
        logSuccess(`✅ Palabras baneadas cargadas: ${palabrasBaneadas.length}`);
    } catch(e) {
        logError('Error cargando palabras baneadas:', e);
    }
    
    logInfo(`🔒 Bloqueos activos: ${bloqueos.size}`);
}
...
app.use((req, res, next) => {
    ...
    if (estaBloqueado(ip)) {
        ...
        logWarn(`🚫 Acceso denegado a IP bloqueada: ${ip} - ${bloqueo?.motivo}`);
...
app.get('/admin-emergencia/:token', (req, res) => {
    ...
    if (token === 'SERENITY2024') {
        ...
        if (bloqueos.has(ipLimpia)) {
            bloqueos.delete(ipLimpia);
            logSuccess(`✅ IP ${ipLimpia} desbloqueada por emergencia`);
...
// ============================================================
// CONFIGURACIÓN PRINCIPAL (INDEX)
// ============================================================
app.get('/api/config', (req, res) => {
    logInfo('📤 GET /api/config - Enviando configuración principal');
    res.json(configData);
});
...
app.put('/api/config', async (req, res) => {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        logWarn('🔒 PUT /api/config - No autorizado (sin token)');
        return res.status(401).json({ error: 'No autorizado' });
    }
    const token = authHeader.substring(7);
    if (!validTokens.has(token) || validTokens.get(token) < Date.now()) {
        logWarn('🔒 PUT /api/config - Token expirado o inválido');
        return res.status(401).json({ error: 'Sesión expirada' });
    }
    
    try {
        const data = req.body;
        logInfo('📝 PUT /api/config - Recibiendo configuración', data);
        ...
        logSuccess('✅ Configuración principal guardada correctamente');
        res.json({ success: true, data: configData });
    } catch(e) {
        logError('Error guardando configuración:', e);
...
app.post('/api/upload-hero', async (req, res) => {
    ...
    try {
        ...
        logSuccess(`✅ Imagen del hero subida: ${url}`);
        res.json({ success: true, url });
    } catch(e) {
        logError('Error subiendo imagen:', e);
...
app.get('/api/servicios', (req, res) => {
    try {
        ...
        logError('Error al obtener servicios:', e);
...
app.post('/api/servicios', async (req, res) => {
    ...
    try {
        ...
        logError('Error al crear el servicio:', e);
...
app.put('/api/servicios/:id', async (req, res) => {
    ...
    try {
        ...
        logError('Error al actualizar el servicio:', e);
...
app.delete('/api/servicios/:id', async (req, res) => {
    ...
    try {
        ...
        logError('Error al eliminar el servicio:', e);
...
app.put('/api/config/horarios', async (req, res) => {
    ...
    try {
        ...
        logError('Error al guardar horarios:', e);
...
// ============================================================
// ============================================================
// CONFIGURACIÓN REGISTRO - CON SUPABASE COMO PRINCIPAL
// ============================================================
...
function normalizeRegistroConfig(data) {
    logInfo('🔄 Normalizando configuración de registro', data);
    ...
    logSuccess('✅ Configuración normalizada correctamente');
    return data;
}

// ============================================================
// FUNCIONES DE LECTURA/ESCRITURA CON SUPABASE
// ============================================================

async function getRegistroConfig() {
    logInfo('📖 Obteniendo configuración de registro...');
    
    // PRIMERO: Intentar desde Supabase (si está disponible)
    if (supabase) {
        try {
            logInfo('📖 Intentando leer desde Supabase...');
            ...
            if (!error && data) {
                logSuccess('✅ Configuración obtenida de Supabase');
                logInfo('📋 Datos de Supabase - Hero:', data.hero?.titulo);
                logInfo('📋 Datos de Supabase - Beneficio 1:', data.beneficios?.[0]?.descripcion);
                return data;
            } else if (error) {
                logWarn('⚠️ Error leyendo de Supabase:', error.message);
                // Si el error es que no hay registros, intentar crear uno
                if (error.code === 'PGRST116') {
                    logInfo('📝 No hay registros en Supabase, creando uno por defecto...');
                    const defaultData = getDefaultRegistroConfig();
                    const { error: insertError } = await supabase
                        .from(SUPABASE_TABLE)
                        .insert({
                            hero: defaultData.hero,
                            beneficios: defaultData.beneficios,
                            conceptos: defaultData.conceptos,
                            imagenWhatsApp: defaultData.imagenWhatsApp,
                            created_at: new Date().toISOString(),
                            updated_at: new Date().toISOString()
                        });
                    
                    if (!insertError) {
                        logSuccess('✅ Registro por defecto creado en Supabase');
                        return defaultData;
                    } else {
                        logError('❌ Error creando registro por defecto:', insertError);
                    }
                }
            }
        } catch (e) {
            logWarn('⚠️ Error en getRegistroConfig de Supabase:', e.message);
        }
    }
    
    // SEGUNDO: Intentar desde archivo local
    try {
        if (fsSync.existsSync(REGISTRO_FILE)) {
            logInfo('📖 Leyendo archivo local:', REGISTRO_FILE);
            const fileContent = await fs.readFile(REGISTRO_FILE, 'utf8');
            const data = JSON.parse(fileContent);
            logSuccess('✅ Configuración leída del archivo local');
            logInfo('📋 Datos del archivo local - Beneficio 1:', data.beneficios?.[0]?.descripcion);
            return data;
        } else {
            logWarn('⚠️ Archivo local no existe:', REGISTRO_FILE);
        }
    } catch (e) {
        logError('Error leyendo archivo local de registro:', e);
    }
    
    // TERCERO: Usar valores por defecto
    logWarn('⚠️ Usando configuración por defecto');
    return getDefaultRegistroConfig();
}

async function saveRegistroConfig(data) {
    logInfo('💾 Guardando configuración de registro...');
    logInfo('📋 Datos a guardar - Beneficio 1:', data.beneficios?.[0]?.descripcion);
    
    try {
        const normalizedData = normalizeRegistroConfig(data);
        logInfo('📋 Datos normalizados - Beneficio 1:', normalizedData.beneficios?.[0]?.descripcion);
        
        // SIEMPRE guardar en archivo local (fallback)
        await fs.writeFile(REGISTRO_FILE, JSON.stringify(normalizedData, null, 2));
        logSuccess('✅ Configuración guardada en archivo local');
        
        // VERIFICAR que se guardó correctamente
        try {
            const verifyContent = await fs.readFile(REGISTRO_FILE, 'utf8');
            const verifyData = JSON.parse(verifyContent);
            logSuccess('✅ Verificación de guardado exitosa');
            logInfo('📋 Beneficio 1 verificado:', verifyData.beneficios?.[0]?.descripcion);
        } catch (verifyError) {
            logError('❌ Error verificando guardado:', verifyError);
        }
        
        // GUARDAR EN SUPABASE (si está disponible)
        if (supabase) {
            try {
                logInfo('💾 Intentando guardar en Supabase...');
                ...
                if (existingData && existingData.id) {
                    logInfo('📝 Actualizando registro existente en Supabase (ID: ' + existingData.id + ')');
                    ...
                    logInfo('📝 Creando nuevo registro en Supabase');
                    result = await supabase
                        .from(SUPABASE_TABLE)
                        .insert({
                            hero: normalizedData.hero,
                ...
                if (result.error) {
                    logError('❌ Error guardando en Supabase:', result.error.message);
                } else {
                    logSuccess('✅ Configuración guardada en Supabase correctamente');
                    
                    // Verificar que se guardó en Supabase
                    const { data: verifySupabase, error: verifyError } = await supabase
                        .from(SUPABASE_TABLE)
                        .select('*')
                        .order('created_at', { ascending: false })
                        .limit(1)
                    ...
                    if (!verifyError && verifySupabase) {
                        logSuccess('✅ Verificación en Supabase exitosa');
                        logInfo('📋 Beneficio 1 en Supabase:', verifySupabase.beneficios?.[0]?.descripcion);
                    }
                }
            } catch (e) {
                logError('❌ Error en operación de Supabase:', e.message);
            }
        } else {
            logWarn('⚠️ Supabase no disponible, solo se guardó en archivo local');
        }
...
app.get('/api/config/registro', async (req, res) => {
    logInfo('📤 GET /api/config/registro - Solicitud recibida');
    try {
        const data = await getRegistroConfig();
        const normalized = normalizeRegistroConfig(data);
        
        logSuccess('📤 Enviando configuración de registro');
        logInfo('📋 Hero:', normalized.hero);
        logInfo('📋 Beneficio 1:', normalized.beneficios?.[0]?.descripcion);
        logInfo('📋 Concepto 1:', normalized.conceptos?.[0]?.titulo);
        
        res.json(normalized);
    } catch (e) {
        logError('❌ Error en GET /api/config/registro:', e);
...
app.put('/api/config/registro', async (req, res) => {
    logInfo('📥 PUT /api/config/registro - Solicitud recibida');
    
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        logWarn('🔒 PUT /api/config/registro - No autorizado (sin token)');
        return res.status(401).json({ error: 'No autorizado' });
    }
    const token = authHeader.substring(7);
    if (!validTokens.has(token) || validTokens.get(token) < Date.now()) {
        logWarn('🔒 PUT /api/config/registro - Token expirado o inválido');
        return res.status(401).json({ error: 'Sesión expirada' });
    }
    
    try {
        const data = req.body;
        logInfo('📝 Datos recibidos para guardar:');
        logInfo('  - Hero:', data.hero?.titulo);
        logInfo('  - Beneficio 1:', data.beneficios?.[0]?.descripcion);
        logInfo('  - Beneficio 2:', data.beneficios?.[1]?.descripcion);
        
        if (!data || typeof data !== 'object') {
            logError('❌ Datos inválidos: no es un objeto');
            return res.status(400).json({ success: false, error: 'Datos inválidos' });
        }
        
        // Validar estructura
        if (!data.hero) {
            logWarn('⚠️ No se recibió hero, usando valores por defecto');
            data.hero = {
                titulo: 'El Arte del Masaje Profesional',
                subtitulo: 'Domina las técnicas que transforman cuerpos y vidas',
                boton: 'Comenzar Ahora'
        ...
        if (!data.beneficios || !Array.isArray(data.beneficios) || data.beneficios.length !== 6) {
            logWarn('⚠️ No se recibieron los 6 beneficios, usando valores por defecto');
            data.beneficios = [
                { descripcion: 'Porcentaje de descuento en todos los servicios (configurable por admin)' },
                { descripcion: 'Reserva paquetes de 5, 10, o 15 días consecutivos con precio especial' },
                { descripcion: 'Cada año de membresía activa, recibe un masaje completamente gratis' },
        ...
        if (!data.conceptos || !Array.isArray(data.conceptos) || data.conceptos.length !== 6) {
            logWarn('⚠️ No se recibieron los 6 conceptos, usando valores por defecto');
            data.conceptos = [
                { titulo: 'Masaje Sueco', descripcion: 'Movimientos largos y fluidos para relajar los músculos superficiales.' },
                { titulo: 'Tejido Profundo', descripcion: 'Trabaja en capas musculares profundas. Ideal para contracturas crónicas.' },
                { titulo: 'Masaje Deportivo', descripcion: 'Pre-competencia, post-competencia y mantenimiento para atletas.' },
        ...
        logInfo('📝 Datos validados, guardando...');
        const success = await saveRegistroConfig(data);
        
        if (success) {
            // Obtener los datos guardados para verificar
            const savedData = await getRegistroConfig();
            const normalized = normalizeRegistroConfig(savedData);
            
            logSuccess('✅ Configuración guardada correctamente');
            logInfo('📋 Datos guardados - Beneficio 1:', normalized.beneficios?.[0]?.descripcion);
            logInfo('📋 Datos guardados - Beneficio 2:', normalized.beneficios?.[1]?.descripcion);
            ...
            logError('❌ Error al guardar la configuración');
            res.status(500).json({
                success: false,
        ...
        logError('❌ Error guardando configuración de registro:', e);
        res.status(500).json({
            success: false,
...
app.post('/turnos', async (req, res) => {
    ...
    logInfo('📝 POST /turnos - Solicitud recibida', { nombre, dia, hora, massageType, ip });
    
    if (estaBloqueado(ip)) {
        logWarn('🚫 IP bloqueada intentando reservar:', ip);
    ...
    logSuccess('✅ Turno creado correctamente', { id: nuevoTurno.id, nombre: nuevoTurno.nombre });
...
app.delete('/turnos/:id', async (req, res) => {
    ...
    try {
        ...
        logError('Error al eliminar turno:', e);
...
app.post('/api/login', (req, res) => {
    ...
    if (password === adminPassword) {
        ...
        logWarn('❌ Login fallido - Contraseña incorrecta');
...
app.get('/api/verify', (req, res) => {
    ...
    logInfo('🔐 Verificación de token:', { valid });
    res.json({ valid });
});
...
app.post('/api/seguridad/desbloquear/:ip', (req, res) => {
    ...
    if (bloqueos.has(ipDesbloquear)) {
        ...
        logSuccess(`🔓 IP ${ipDesbloquear} desbloqueada`);
...
app.post('/api/seguridad/bloquear-usuario/:id', async (req, res) => {
    ...
    try {
        ...
        logError('Error al bloquear usuario:', e);
...
app.put('/api/seguridad/paises', async (req, res) => {
    ...
    try {
        ...
        logError('Error al guardar países:', e);
        res.status(500).json({ error: 'Error al guardar' });
    }
});
...
app.post('/api/seguridad/paises/autorizar', async (req, res) => {
    ...
    try {
        ...
        logError('Error al autorizar país:', e);
...
app.post('/api/seguridad/paises/bloquear', async (req, res) => {
    ...
    try {
        ...
        logError('Error al bloquear país:', e);
...
app.delete('/api/seguridad/paises/:codigo', async (req, res) => {
    ...
    try {
        ...
        logError('Error al eliminar país:', e);
...
app.post('/api/enviar-whatsapp/:id', async (req, res) => {
    ...
    try {
        ...
        logError('Error preparando WhatsApp:', e);
...
app.post('/api/palabras-baneadas', async (req, res) => {
    ...
    try {
        ...
        logError('Error al modificar palabras baneadas:', e);
...
app.put('/api/ia/personalidad', async (req, res) => {
    ...
    try {
        ...
        logError('Error al guardar personalidad IA:', e);
...
app.get('/api/seguridad/estado/current', (req, res) => {
    ...
    logInfo('🔍 Verificando estado de bloqueo para IP:', ipLimpia);
    
    if (bloqueos.has(ipLimpia)) {
        const datos = bloqueos.get(ipLimpia);
        const tiempoRestante = Math.max(0, datos.hasta - Date.now());
        
        logWarn('🚫 IP bloqueada:', { ip: ipLimpia, motivo: datos.motivo, permanente: datos.permanente });
        ...
        res.json({
            ...
            datos: {
                ...
                tiempoRestanteFormateado: formatearTiempoRestante(tiempoRestante),
                palabraOfensiva: datos.palabraOfensiva || null,
                conteoBloqueos: datos.conteoBloqueos || 0
            }
        });
    } else {
        logInfo('✅ IP no bloqueada:', ipLimpia);
        res.json({ bloqueado: false });
    }
});

// ============================================================
// SEGURIDAD - VERIFICAR VPN
// ============================================================
app.get('/api/seguridad/verificar-vpn', (req, res) => {
    const ip = req.ip || req.socket.remoteAddress || '0.0.0.0';
    const ipLimpia = ip.replace('::ffff:', '');
    ...
    const ipsSospechosas = ['192.168.1.100', '10.0.0.1', '172.16.0.1'];
    const esSospechosa = ipsSospechosas.includes(ipLimpia);
...
// ============================================================
app.post('/api/seguridad/infraccion', (req, res) => {
    const ip = req.ip || req.socket.remoteAddress || '0.0.0.0';
    const ipLimpia = ip.replace('::ffff:', '');
    const { tipo, detalle } = req.body;
    
    logInfo('📝 Registrando infracción:', { ip: ipLimpia, tipo, detalle });
    
    if (!infraccionesPorIP.has(ipLimpia)) {
        infraccionesPorIP.set(ipLimpia, { count: 0, infracciones: [] });
    }
    const infraccion = infraccionesPorIP.get(ipLimpia);
    infraccion.count += 1;
    infraccion.infracciones.push({ tipo, detalle, fecha: new Date().toISOString() });
    
    logWarn(`⚠️ Infracción ${infraccion.count} para IP ${ipLimpia}: ${tipo}`);
    
    if (infraccion.count >= 3) {
        const motivo = `Múltiples infracciones (${infraccion.count}): ${tipo}`;
        bloquearIPConAcumulativo(ipLimpia, motivo, 'INFRACCION', detalle, false);
        
        return res.json({
            bloqueado: true,
            permanente: false,
            motivo: `Has acumulado ${infraccion.count} infracciones. Bloqueo temporal aplicado.`,
            restantes: 0
        });
    }
    
    const restantes = 3 - infraccion.count;
    res.json({
        bloqueado: false,
        advertencia: true,
        mensaje: `Infracción registrada: ${tipo}`,
        restantes: restantes,
        count: infraccion.count
    });
});

// ============================================================
// PREMIUM - REGISTRAR USUARIO
// ============================================================
app.post('/api/premium/usuario', async (req, res) => {
    try {
        logInfo('📝 Registro premium solicitado:', req.body);
        res.json({ success: true, mensaje: 'Usuario registrado correctamente' });
    } catch (e) {
        logError('Error registrando usuario premium:', e);
        res.status(500).json({ success: false, error: 'Error al registrar usuario' });
    }
});

// ============================================================
// RUTA DE DEBUG - Ver contenido del archivo registro-config.json
// ============================================================
...
// ============================================================
// RUTA PARA VER DATOS DE SUPABASE DIRECTAMENTE
// ============================================================
app.get('/api/debug/supabase', async (req, res) => {
    if (!supabase) {
        return res.json({ error: 'Supabase no disponible' });
    }
...
const server = app.listen(PORT, '0.0.0.0', async () => {
    await initAllFiles();
    console.log(`${colors.bright}${colors.green}🌿 Serenity Spa iniciado en puerto ${PORT}${colors.reset}`);
    console.log(`${colors.blue}📂 Directorio base: ${BASE_DIR}${colors.reset}`);
    console.log(`${colors.blue}📂 Directorio de datos: ${DATA_DIR}${colors.reset}`);
    console.log(`${colors.blue}📋 Servicios disponibles: ${serviciosData.length}${colors.reset}`);
    console.log(`${colors.blue}🔒 Bloqueos activos: ${bloqueos.size}${colors.reset}`);
    console.log(`${colors.blue}📍 Ubicación del salón: ${ubicacionSalonGlobal}${colors.reset}`);
    console.log(`${colors.blue}🔗 Supabase: ${supabase ? '✅ Conectado' : '❌ No disponible (usando archivos locales)'}${colors.reset}`);
    console.log(`${colors.bright}${colors.yellow}📝 Logs detallados activados - Revisa los logs para depurar${colors.reset}`);
    console.log(`${colors.bright}${colors.cyan}🔍 Rutas de debug disponibles:${colors.reset}`);
    console.log(`${colors.cyan}   /api/debug/registro - Ver contenido del archivo local${colors.reset}`);
    console.log(`${colors.cyan}   /api/debug/supabase - Ver datos en Supabase${colors.reset}`);
});
