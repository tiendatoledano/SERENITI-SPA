...
// server.js - Función WhatsApp CORREGIDA (sin forzar extensión)
    ...
    try {
        ...
        try {
            ...
            if (servicio && servicio.imagen_whatsapp) {
                let imgUrl = servicio.imagen_whatsapp.trim();
                
                // ✅ SOLO CONVERTIR si es URL de postimg.cc que no es directa
                if (imgUrl.includes('postimg.cc') && !imgUrl.includes('i.postimg.cc')) {
                    const match = imgUrl.match(/postimg\.cc\/([a-zA-Z0-9]+)/);
                    if (match) {
                        const code = match[1];
                        // ✅ Usar la URL tal cual, sin forzar .jpg
                        imgUrl = `https://i.postimg.cc/${code}`;
                        console.log(`📸 URL convertida: ${imgUrl}`);
                    }
                }
                
                // ✅ NO FORZAR EXTENSIÓN .jpg - Mantener la URL exacta del usuario
