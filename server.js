...
// ==================== FUNCIÓN PARA OBTENER FECHA PRÓXIMA DISPONIBLE ====================
function obtenerProximaFechaDisponible(diaSolicitado, turnosExistentes) {
    const diasSemana = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado'];
    const hoy = new Date();
    const diaActual = diasSemana[hoy.getDay()];
    
    // Si el día solicitado no es válido, buscar el próximo día disponible
    ...
    // Verificar si hay horarios disponibles para el día solicitado
    ...
    // Buscar el próximo día con horarios disponibles
    for (let i = 1; i <= 7; i++) {
        const fecha = new Date();
        fecha.setDate(fecha.getDate() + i);
        const diaSemana = diasSemana[fecha.getDay()];
...
async function initAllFiles() {
    ...
    if (!fsSync.existsSync(SERVICIOS_FILE)) {
        const serviciosDefault = [
            { id: generarId(), nombre: "Masaje Relajante", precio: "$45", descripcion: "Movimientos suaves para liberar el estrés.", beneficios: ["60 Minutos"], efectos: ["Relajación profunda"], imagenWeb: "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?w=800", orden: 1 },
            { id: generarId(), nombre: "Masaje Corporal", precio: "$65", descripcion: "Tratamiento completo para una relajación profunda.", beneficios: ["90 Minutos"], efectos: ["Activación linfática"], imagenWeb: "https://images.unsplash.com/photo-1519823551278-64ac92734fb1?w=800", orden: 2 },
            { id: generarId(), nombre: "Masaje Facial", precio: "$40", descripcion: "Rejuvenece la piel y alivia la tensión facial.", beneficios: ["45 Minutos"], efectos: ["Estimula colágeno"], imagenWeb: "https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?w=800", orden: 3 }
        ];
...
app.post('/turnos', async (req, res) => {
    ...
    // Procesar día y hora con validación mejorada
    let diaLower = dia?.toLowerCase();
    let horaNumerica = parseInt(hora);
    
    // Si no se proporcionó día o hora, sugerir el próximo disponible
    if (!diaLower || !DIAS_VALIDOS.includes(diaLower) || isNaN(horaNumerica) || !HORAS_VALIDAS.includes(horaNumerica)) {
        const proximaFecha = obtenerProximaFechaDisponible(diaLower, turnos);
        if (proximaFecha) {
            return res.status(409).json({
                error: 'Fecha u horario no válido',
                sugerencia: {
                    dia: proximaFecha.dia,
                    hora: proximaFecha.hora,
                    fechaTexto: formatearFecha(proximaFecha.fecha)
                },
                mensaje: `El día ${diaLower || 'seleccionado'} no está disponible. Te sugiero el ${formatearFecha(proximaFecha.fecha)} a las ${proximaFecha.hora}:00 hs.`
            });
        }
        return res.status(400).json({ 
            error: 'Día u hora inválida. Por favor, selecciona un día y hora válidos.',
            sugerencia: 'Usa el formato: lunes, martes, miércoles, jueves, viernes o sábado'
        });
    }
    ...
    const hoy = new Date();
    const diaActual = diasSemana[hoy.getDay()];
    const idxSolicitado = DIAS_VALIDOS.indexOf(diaLower);
    const idxHoy = DIAS_VALIDOS.indexOf(diaActual);
    ...
    if (idxSolicitado < idxHoy) {
        ...
        if (proximaFecha) {
            return res.status(409).json({
                ...
                mensaje: `El día ${diaLower} ya pasó. Te sugiero el ${formatearFecha(proximaFecha.fecha)} a las ${proximaFecha.hora}:00 hs.`
            });
        }
        return res.status(400).json({ error: 'El día seleccionado ya pasó. Elige un día futuro.' });
    }
    
    // Verificar si es el mismo día pero la hora ya pasó
    if (idxSolicitado === idxHoy) {
        const horaActual = hoy.getHours();
        if (horaNumerica < horaActual) {
            const proximaFecha = obtenerProximaFechaDisponible(diaLower, turnos);
            if (proximaFecha) {
                return res.status(409).json({
                    ...
                    mensaje: `Las ${horaNumerica}:00 ya pasaron. Te sugiero el ${formatearFecha(proximaFecha.fecha)} a las ${proximaFecha.hora}:00 hs.`
                });
            }
            return res.status(400).json({ error: 'La hora seleccionada ya pasó. Elige otra hora.' });
        }
    }
    ...
    if (turnos.some(t => t.dia === diaLower && t.hora === horaNumerica)) {
        ...
        if (proximaFecha) {
            return res.status(409).json({
                ...
                mensaje: `El ${diaLower} está completo. Te sugiero el ${formatearFecha(proximaFecha.fecha)} a las ${proximaFecha.hora}:00 hs.`
            });
        }
        return res.status(409).json({ error: 'No hay horarios disponibles para el día seleccionado.' });
    }
    
    // Verificar si ya tiene un turno para ese día
...
app.post('/api/enviar-whatsapp/:id', async (req, res) => {
    ...
    try {
        ...
        const mensaje = `✅ Hola ${turno.nombre}, tu turno en Serenity Spa está confirmado.\n\n📅 ${turno.dia} - ${turno.hora}:00 hs\n💆 ${turno.massageType}\n${turno.tipoServicio === 'domicilio' ? `📍 ${turno.ubicacion}` : '📍 En nuestro salón'}\n\n🔑 Código de cancelación: ${turno.codigoCancelacion}\n\n¡Te esperamos!`;
