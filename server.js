...
// ============================================================
// RUTA DE BLOQUEO - DEBE IR ANTES DEL MIDDLEWARE DE BLOQUEO
// ============================================================
...
// ============================================================
// MIDDLEWARE DE BLOQUEO DE IP - SE EJECUTA ANTES DE TODAS LAS RUTAS
// ============================================================
app.use(async (req, res, next) => {
    ...
    // Excluir archivos estáticos críticos
    if (req.path.endsWith('.css') || req.path.endsWith('.js') || req.path.endsWith('.png') || 
        ...
        req.path.endsWith('.json') && req.path.includes('manifest')) {
        return next();
    }
    ...
    try {
        ...
        if (bloqueado) {
            ...
            // Para cualquier otra solicitud, redirigir a la página de bloqueo
...
// ============================================================
// CONFIGURACIÓN EMAILJS (para el frontend)
// ============================================================
...
app.post('/api/registro/verificar-codigo', async (req, res) => {
    try {
        ...
        // ⭐ CORREGIDO: No incluir 'ip' en el objeto
        const nuevoUsuario = {
            id: generarId(),
...
// Endpoint específico para actualizar solo la ubicación del salón
...
// ============================================================
// BLOQUEO/DESBLOQUEO DE IPs POR TURNO - CORREGIDO
// ============================================================

// Bloquear IP de un turno específico
app.post('/api/seguridad/bloquear-usuario-ip/:id', verifyAuth, async (req, res) => {
    try {
        ...
        // Validar que sea una IP real (no local/interna)
        const esIPLocal = (ip) => {
        ...
        // Incrementar contador de bloqueos del turno
        ...
        // Bloquear la IP en el sistema de seguridad (30 días)
...
// Bloquear una IP específica (manual)
