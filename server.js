...
const REGISTRO_FILE = path.join(__dirname, 'registro.html');
...
function initSupabase() {
    try {
        ...
        if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
            ...
            console.warn('   Configura: SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY');
            console.warn('   Las rutas de contenido usarán datos por defecto');
            return;
        }
        ...
        console.warn('⚠️ Error al cargar @supabase/supabase-js:', error.message);
        console.warn('   Ejecuta: npm install @supabase/supabase-js');
        console.warn('   Las rutas de contenido usarán datos por defecto');
        supabase = null;
    }
}
...
function obtenerProximaFechaDisponible(diaSolicitado, turnosExistentes) {
    const diasSemana = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado'];
    const hoy = new Date();
    const diaActual = diasSemana[hoy.getDay()];
    
    let diaBuscado = diaSolicitado?.toLowerCase();
    
    if (diaBuscado && DIAS_VALIDOS.includes(diaBuscado)) {
        const idxSolicitado = DIAS_VALIDOS.indexOf(diaBuscado);
        const idxHoy = DIAS_VALIDOS.indexOf(diaActual);
        
        if (idxSolicitado < idxHoy) {
            for (let i = 1; i <= 7; i++) {
                const fecha = new Date();
                fecha.setDate(fecha.getDate() + i);
                const diaSemana = diasSemana[fecha.getDay()];
                if (DIAS_VALIDOS.includes(diaSemana)) {
                    const horariosLibres = HORAS_VALIDAS.filter(h => 
                        !turnosExistentes.some(t => t.dia === diaSemana && t.hora === h)
                    );
                    if (horariosLibres.length > 0) {
                        return {
                            dia: diaSemana,
                            fecha: fecha,
                            hora: horariosLibres[0],
                            disponible: true,
        ...
        const horariosDisponibles = HORAS_VALIDAS.filter(h => 
            !turnosExistentes.some(t => t.dia === diaBuscado && t.hora === h)
        );
...
app.use((req, res, next) => {
    ...
    const rutasPermitidas = [
        ...
        '/api/recargar', '/api/enviar-whatsapp',
        '/api/registro/contenido', '/api/registro/reset',
        '/api/contenido/textos', '/api/contenido/beneficios',
        '/api/contenido/conceptos', '/api/contenido/estadisticas',
        '/api/chat-ia', '/api/ia/personalidad', '/api/ia/recargar',
        '/api/config-frontend', '/api/premium/usuario'
    ];
...
// ==================== RUTAS PARA EDITAR REGISTRO.HTML ====================

app.get('/api/registro/contenido', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        const contenido = await fs.readFile(REGISTRO_FILE, 'utf8');
        res.json({ contenido });
    } catch(e) {
        console.error('Error al leer registro.html:', e);
        res.status(500).json({ error: 'Error al leer el archivo' });
    }
});

app.put('/api/registro/contenido', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        const backupPath = path.join(__dirname, `registro-backup-${Date.now()}.html`);
        try {
            const contenidoActual = await fs.readFile(REGISTRO_FILE, 'utf8');
            await fs.writeFile(backupPath, contenidoActual, 'utf8');
        } catch(e) {}
        
        await fs.writeFile(REGISTRO_FILE, contenido, 'utf8');
        console.log(`✅ registro.html actualizado - Backup: ${path.basename(backupPath)}`);
        res.json({ success: true, mensaje: 'Archivo actualizado correctamente' });
    } catch(e) {
        console.error('Error al guardar registro.html:', e);
        res.status(500).json({ error: 'Error al guardar el archivo' });
    }
});

app.post('/api/registro/reset', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        if (!fsSync.existsSync(REGISTRO_FILE)) {
            return res.status(404).json({ error: 'El archivo registro.html no existe' });
        }
        
        const backupPath = path.join(__dirname, `registro-backup-reset-${Date.now()}.html`);
        const contenidoActual = await fs.readFile(REGISTRO_FILE, 'utf8');
        await fs.writeFile(backupPath, contenidoActual, 'utf8');
        ...
        console.error('Error al resetear registro.html:', e);
        res.status(500).json({ error: 'Error al crear backup' });
    }
});

// ============================================================
// RUTAS PARA CONTENIDO (Beneficios, Conceptos, Textos, Estadísticas)
// ============================================================

// ---------- TEXTOS ----------
app.get('/api/contenido/textos', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    if (!supabase) {
        return res.status(503).json({ error: 'Supabase no disponible' });
    }

    try {
        const { data, error } = await supabase
            .from('secciones_texto')
            .select('*');
        ...
        const result = {};
        data.forEach(item => {
            result[item.seccion] = item.contenido;
        });
        res.json(result);
    } catch(e) {
        console.error('Error cargando textos:', e);
        res.status(500).json({ error: 'Error al cargar textos' });
    }
});

app.put('/api/contenido/textos', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        for (const [seccion, contenido] of Object.entries(textos)) {
            const { error } = await supabase
                .from('secciones_texto')
                .upsert({ seccion, contenido }, { onConflict: 'seccion' });
        ...
        res.status(500).json({ error: 'Error al guardar textos' });
    }
});

// ---------- BENEFICIOS ----------
app.get('/api/contenido/beneficios', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        res.status(500).json({ error: 'Error al cargar beneficios' });
    }
});

app.post('/api/contenido/beneficios', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        console.error('Error creando beneficio:', e);
        res.status(500).json({ error: 'Error al crear beneficio' });
    }
});

app.put('/api/contenido/beneficios/:id', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
...
app.delete('/api/contenido/beneficios/:id', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        console.error('Error eliminando beneficio:', e);
        res.status(500).json({ error: 'Error al eliminar beneficio' });
    }
});

app.post('/api/contenido/beneficios/:id/toggle', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        console.error('Error togglando beneficio:', e);
        res.status(500).json({ error: 'Error al actualizar estado' });
    }
});

// ---------- CONCEPTOS ----------
app.get('/api/contenido/conceptos', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        res.status(500).json({ error: 'Error al cargar conceptos' });
    }
});

app.post('/api/contenido/conceptos', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        const { numero, titulo, descripcion, etiqueta, icono, orden } = req.body;
        const { data, error } = await supabase
            .from('conceptos')
            .insert({ numero, titulo, descripcion, etiqueta, icono, orden })
            .select()
            .single();
...
app.put('/api/contenido/conceptos/:id', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        const { data, error } = await supabase
            .from('conceptos')
            .update({ numero, titulo, descripcion, etiqueta, icono, orden })
            .eq('id', id)
            .select()
            .single();
...
app.delete('/api/contenido/conceptos/:id', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
...
app.post('/api/contenido/conceptos/:id/toggle', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        console.error('Error togglando concepto:', e);
        res.status(500).json({ error: 'Error al actualizar estado' });
    }
});
...
app.post('/api/contenido/estadisticas', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        const { numero, label, orden } = req.body;
        const { data, error } = await supabase
            .from('estadisticas')
            .insert({ numero, label, orden })
            .select()
            .single();
        ...
        console.error('Error creando estadística:', e);
        res.status(500).json({ error: 'Error al crear estadística' });
    }
});

app.put('/api/contenido/estadisticas/:id', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        console.error('Error actualizando estadística:', e);
        res.status(500).json({ error: 'Error al actualizar estadística' });
    }
});

app.delete('/api/contenido/estadisticas/:id', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
...
app.post('/api/contenido/estadisticas/:id/toggle', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        console.error('Error togglando estadística:', e);
        res.status(500).json({ error: 'Error al actualizar estado' });
    }
});
...
let iaPersonalidad = {
    ...
    estilo: 'Hablar en español neutro, ser amable y empático con los clientes.',
    reglas: [
        'NUNCA inventar información sobre precios o disponibilidad.',
        'SIEMPRE ofrecer reservar turnos cuando el cliente lo solicite.',
        'SIEMPRE preguntar el nombre del cliente al inicio de la conversación.'
    ]
};
...
app.put('/api/ia/personalidad', async (req, res) => {
    ...
    try {
        ...
        iaPersonalidad = {
            nombre: nombre || iaPersonalidad.nombre,
            tono: tono || iaPersonalidad.tono,
            estilo: estilo || iaPersonalidad.estilo,
            reglas: reglas || iaPersonalidad.reglas
        };
        res.json({ success: true, personalidad: iaPersonalidad });
    } catch(e) {
        res.status(500).json({ error: 'Error al guardar personalidad' });
    }
});
...
app.post('/api/ia/recargar', (req, res) => {
    ...
    res.json({ success: true, items: serviciosData.length });
});
...
app.post('/turnos', async (req, res) => {
    ...
    console.log(`📋 Datos recibidos:`, { nombre, telefono, massageType, dia, hora, codigoPais, tipoServicio });
    ...
    if (!diaLower || !DIAS_VALIDOS.includes(diaLower)) {
        const proximaFecha = obtenerProximaFechaDisponible(diaLower, turnos);
        if (proximaFecha) {
            return res.status(409).json({
                error: 'Día no válido',
                sugerencia: {
                    dia: proximaFecha.dia,
                    hora: proximaFecha.hora,
                    fechaTexto: proximaFecha.fechaTexto
                },
                mensaje: `El día ${diaLower || 'seleccionado'} no está disponible. Te sugiero ${proximaFecha.fechaTexto} a las ${proximaFecha.hora}:00 hs.`
            });
        }
    ...
    if (!esFechaValida(diaLower)) {
        ...
        if (proximaFecha) {
            return res.status(409).json({
                error: 'El día seleccionado ya pasó',
                sugerencia: {
                    dia: proximaFecha.dia,
                    hora: proximaFecha.hora,
                    fechaTexto: proximaFecha.fechaTexto
                },
                mensaje: `Usted menciona el día ${diaLower} pero ese día ya pasó. Le puedo recomendar ${proximaFecha.fechaTexto} a las ${proximaFecha.hora}:00 hs.`
            });
        }
        return res.status(400).json({ 
            error: 'El día seleccionado ya pasó',
            mensaje: `El día ${diaLower} ya pasó. Por favor, selecciona un día futuro.`
        });
    }
    
    if (isNaN(horaNumerica) || !HORAS_VALIDAS.includes(horaNumerica)) {
        const horariosDisponibles = HORAS_VALIDAS.filter(h => 
            !turnos.some(t => t.dia === diaLower && t.hora === h)
        );
        if (horariosDisponibles.length > 0) {
            return res.status(400).json({
                ...
                sugerencia: {
                    dia: diaLower,
                    hora: horariosDisponibles[0],
                    fechaTexto: `el ${diaLower}`
                },
                mensaje: `Hora no válida. Horarios disponibles: ${horariosDisponibles.join(' hs, ')} hs.`
            });
        }
        return res.status(400).json({ 
            error: 'Hora no válida',
    ...
    if (turnos.some(t => t.dia === diaLower && t.hora === horaNumerica)) {
        const horariosDisponibles = HORAS_VALIDAS.filter(h => 
            !turnos.some(t => t.dia === diaLower && t.hora === h)
        );
        ...
        if (horariosDisponibles.length > 0) {
            return res.status(409).json({
                ...
                mensaje: `El horario de las ${horaNumerica}:00 del ${diaLower} está ocupado. ¿Te gustaría a las ${horariosDisponibles[0]}:00?`
            });
        }
        ...
        if (proximaFecha) {
            return res.status(409).json({
                ...
                mensaje: `El ${diaLower} está completo. Te sugiero ${proximaFecha.fechaTexto} a las ${proximaFecha.hora}:00 hs.`
            });
        }
        return res.status(409).json({ error: 'No hay horarios disponibles para el día seleccionado.' });
    }
    ...
    console.log(`✅ Turno reservado: ${nuevoTurno.nombre} - ${diaLower} ${horaNumerica}:00`);
...
app.post('/api/enviar-whatsapp/:id', async (req, res) => {
    ...
    try {
        ...
        const ubicacionTexto = turno.tipoServicio === 'domicilio' ? `📍 Dirección: ${turno.ubicacion}` : '📍 Serenity Spa';
...
app.post('/api/premium/usuario', async (req, res) => {
    try {
        const { email, nombre, telefono, codigoPais } = req.body;
        ...
        if (supabase) {
            try {
                ...
                if (error) throw error;
                console.log(`✅ Usuario premium guardado en Supabase: ${email}`);
            } catch(e) {
        ...
        console.error('Error en /api/premium/usuario:', e);
...
const server = app.listen(PORT, '0.0.0.0', async () => {
    ...
    console.log(`📦 Supabase: ${supabase ? '✅ Conectado' : '❌ No disponible'}`);
    console.log(`🤖 DeepSeek IA: ${process.env.DEEPSEEK_API_KEY ? '✅ Configurada' : '❌ No configurada'}`);
});
