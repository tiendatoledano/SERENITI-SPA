// ============================================================
// server.js - SERVIDOR PRINCIPAL CON SUPABASE (CORREGIDO)
// ============================================================
...
// ============================================================
// CACHE DE SERVICIOS PARA IMÁGENES
// ============================================================
let serviciosCache = [];
let serviciosCacheTimestamp = 0;
const SERVICIOS_CACHE_TTL = 60000; // 1 minuto

async function getServiciosCache() {
    const ahora = Date.now();
    if (serviciosCache.length > 0 && (ahora - serviciosCacheTimestamp) < SERVICIOS_CACHE_TTL) {
    ...
    try {
        ...
        if (!error && data) {
            serviciosCache = data;
            serviciosCacheTimestamp = ahora;
            console.log(`✅ Servicios cacheados: ${data.length}`);
        }
        return serviciosCache;
    } catch (e) {
        console.warn('Error obteniendo servicios cache:', e);
        return serviciosCache;
    }
}
...
app.get('/api/usuario/actual', async (req, res) => {
    ...
    try {
        ...
        if (usuario) {
            ...
            console.log('⚠️ Usuario no encontrado para token');
...
app.get('/api/verify/user', async (req, res) => {
    ...
    try {
        const usuario = await db.getUsuarioByToken(token);
        if (usuario) {
            console.log('✅ Token válido para:', usuario.nombre);
            return res.json({ 
                valid: true, 
                usuario: { 
                    nombre: usuario.nombre, 
...
app.post('/turnos', async (req, res) => {
    try {
        ...
        // ============================================================
        // ⭐ OBTENER USUARIO AUTENTICADO PARA ASOCIAR EL TURNO
        // ============================================================
        let usuarioId = null;
        ...
        // Buscar precio del servicio si no se proporcionó
        let precioFinal = precio || '';
        if (!precioFinal && massageType) {
            try {
                const servicios = await getServiciosCache();
                const servicio = servicios.find(s => 
                    s.nombre && s.nombre.toLowerCase().includes(massageType.toLowerCase())
                );
    ...
    try {
        ...
        try {
            ...
            if (servicio) {
                ...
                console.log(`📸 Imagen encontrada para ${nombreServicio}: ${imagenWhatsApp}`);
            }
        } catch (e) {
...
app.get('/api/turnos/mis-turnos', async (req, res) => {
    ...
    try {
        ...
        // ⭐ BUSCAR POR usuario_id (prioritario)
        let query = supabase
            .from('turnos')
            .select('*')
        ...
        if (usuario.id) {
            ...
            query = query.eq('email_usuario', usuario.email);
        } else {
            // Fallback: buscar por nombre o teléfono
            if (usuario.telefono) {
        ...
        const turnos = (turnosData || []).map(t => {
            ...
            return {
                ...
                email_usuario: t.email_usuario || null
            };
        });
...
app.post('/api/turnos/confirmar/:id', async (req, res) => {
    ...
    try {
        ...
        // Verificar que el turno pertenece al usuario (por usuario_id o email)
        ...
        if (turno.email_usuario && turno.email_usuario !== usuario.email) {
            return res.status(403).json({ error: 'No tienes permiso para confirmar este turno' });
        }
        if (!turno.usuario_id && !turno.email_usuario) {
            // Fallback: verificar por nombre/telefono
            if (turno.telefono !== usuario.telefono && turno.nombre !== usuario.nombre) {
                return res.status(403).json({ error: 'No tienes permiso para confirmar este turno' });
            }
        }
...
app.post('/api/turnos/cancelar/:id', async (req, res) => {
    ...
    try {
        ...
        // Verificar tiempo (2 horas antes)
        const fechaTurno = new Date(turno.fecha);
        const horaTurno = parseInt(turno.hora);
        fechaTurno.setHours(horaTurno, 0, 0, 0);
        const ahora = new Date();
...
// ⭐ RUTA PARA EJECUTAR VERIFICACIÓN MANUAL (admin)
