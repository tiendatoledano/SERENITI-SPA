...
// ============================================================
// ARCHIVOS Y DIRECTORIOS - CORREGIDO PARA RENDER
// ============================================================
...
// Usar /tmp en Render, o la carpeta actual en desarrollo
...
const INFRACCIONES_FILE = path.join(DATA_DIR, 'infracciones.json');
const BLOQUEOS_HISTORICOS_FILE = path.join(DATA_DIR, 'bloqueos-historicos.json');

// Directorio de uploads (también debe estar en /tmp)
...
// ============================================================
// CONFIGURACIÓN REGISTRO - CORREGIDO
// ============================================================
...
app.get('/api/config/registro', async (req, res) => {
    try {
        let data;
        if (fsSync.existsSync(REGISTRO_FILE)) {
            data = JSON.parse(await fs.readFile(REGISTRO_FILE, 'utf8'));
        } else {
        ...
        // NORMALIZAR BENEFICIOS - Asegurar que cada beneficio tenga la propiedad descripcion
        if (data.beneficios && Array.isArray(data.beneficios)) {
            data.beneficios = data.beneficios.map(b => {
                if (typeof b === 'string') {
                    return { descripcion: b };
                }
                if (b && typeof b === 'object' && b.descripcion) {
                    return { descripcion: b.descripcion };
                }
                // Si no tiene descripcion, usar un valor por defecto
                return { descripcion: 'Beneficio no disponible' };
            });
        } else {
            // Si no hay beneficios, crear los 6 por defecto
            data.beneficios = [
                { descripcion: 'Porcentaje de descuento en todos los servicios (configurable por admin)' },
                { descripcion: 'Reserva paquetes de 5, 10, o 15 días consecutivos con precio especial' },
                { descripcion: 'Cada año de membresía activa, recibe un masaje completamente gratis' },
        ...
        // Asegurar que haya exactamente 6 beneficios
        ...
        if (data.conceptos && Array.isArray(data.conceptos)) {
            data.conceptos = data.conceptos.map(c => {
                if (typeof c === 'string') {
                    return { titulo: 'Concepto', descripcion: c };
                }
                if (c && typeof c === 'object') {
                    return {
                        titulo: c.titulo || 'Concepto',
        ...
        // Asegurar que haya exactamente 6 conceptos
        ...
        // Asegurar que imagenWhatsApp existe
    ...
    try {
        ...
        if (!data.hero || !data.beneficios || !data.conceptos) {
            return res.status(400).json({ 
                success: false, 
                error: 'Datos incompletos - estructura incorrecta' 
            });
        }
        ...
        if (!Array.isArray(data.beneficios) || data.beneficios.length !== 6) {
            return res.status(400).json({ 
                success: false, 
                error: 'Deben haber exactamente 6 beneficios en un array' 
            });
        }
        ...
        if (!Array.isArray(data.conceptos) || data.conceptos.length !== 6) {
            return res.status(400).json({ 
                success: false, 
                error: 'Deben haber exactamente 6 conceptos en un array' 
            });
        }
        
        // Normalizar beneficios: asegurar que cada uno tenga la propiedad descripcion
        data.beneficios = data.beneficios.map(b => {
            if (typeof b === 'string') return { descripcion: b };
            if (!b.descripcion) b.descripcion = '';
            return b;
        });
        
        // Normalizar conceptos: asegurar que cada uno tenga titulo y descripcion
        data.conceptos = data.conceptos.map(c => {
...
// ============================================================
// WHATSAPP - CORREGIDO CON IMAGEN DEL SERVICIO Y FORMATO SOLICITADO
// ============================================================
...
app.post('/api/enviar-whatsapp/:id', async (req, res) => {
    ...
    try {
        ...
        // Buscar el servicio correspondiente para obtener la imagen de WhatsApp
        ...
        if (turno.codigoPais) {
            numero = `${turno.codigoPais}${numero}`;
        }
        
        // Construir mensaje CON EL FORMATO SOLICITADO Y LA IMAGEN DEL SERVICIO
