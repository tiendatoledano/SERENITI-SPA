...
// server.js - Función WhatsApp CORREGIDA
app.post('/api/enviar-whatsapp/:id', verifyAuth, async (req, res) => {
    try {
        ...
        // ====== OBTENER IMAGEN DEL SERVICIO PARA WHATSAPP ======
        ...
        try {
            ...
            if (servicio) {
                ...
                if (servicio.imagen_whatsapp && servicio.imagen_whatsapp.trim() !== '') {
                    let imgUrl = servicio.imagen_whatsapp;
                    // Convertir URL de postimg.cc a i.postimg.cc
                    if (imgUrl.includes('postimg.cc') && !imgUrl.includes('i.postimg.cc')) {
                        imgUrl = imgUrl.replace('postimg.cc', 'i.postimg.cc');
                        // Agregar extensión si no tiene
                        if (!imgUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
                            imgUrl = imgUrl + '.jpg';
                        }
                    }
                    imagenWhatsApp = imgUrl;
                    console.log(`📸 Usando imagen WhatsApp del servicio: ${imagenWhatsApp}`);
                } else if (servicio.imagen_web && servicio.imagen_web.trim() !== '') {
                    // Si no tiene imagen WhatsApp, usar la imagen web (solo si es URL)
                    if (servicio.imagen_web.startsWith('http')) {
                        imagenWhatsApp = servicio.imagen_web;
                        console.log(`📸 Usando imagen Web como fallback: ${imagenWhatsApp}`);
                    }
                }
            }
        } catch (e) {
        ...
        // ====== CONSTRUIR MENSAJE SIN INCLUIR LA URL COMO TEXTO ======
        // ✅ La imagen se envía como media, no como texto
        const mensaje = `🌿 *SERENITY SPA*
...
*Equipo Serenity Spa*`;
        ...
        // ✅ La URL de la imagen se envía por separado, no en el texto
        const whatsappUrl = `https://wa.me/${numero}?text=${encodeURIComponent(mensaje)}`;
        
        await db.updateTurno(id, { confirmado_whatsapp: true });
        
        // ✅ Devolver la imagen para que se muestre en el modal de admin
        res.json({ 
            success: true, 
            numero: numero,
            mensaje: mensaje,
            url: whatsappUrl,
            imagen: imagenWhatsApp  // ✅ Esta URL se usa para la previsualización en el admin
        });
