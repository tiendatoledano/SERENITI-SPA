...
// ==================== SERVICIOS - CORREGIDO COMPLETAMENTE ====================
...
app.put('/api/servicios/:id', async (req, res) => {
    ...
    try {
        ...
        console.log('Datos recibidos:', { nombre, precio, descripcion, beneficios, efectos });
...
app.put('/api/config/horarios', async (req, res) => {
    ...
    try {
        ...
        // ACTUALIZAR DIAS_VALIDOS
        if (data.dias && Array.isArray(data.dias)) {
            horariosConfig.dias = data.dias;
            DIAS_VALIDOS = data.dias; // <--- CRÍTICO: Actualizar la variable global
        }
        
        // ACTUALIZAR HORAS_VALIDAS
        if (data.horarios && Array.isArray(data.horarios)) {
            horariosConfig.horarios = data.horarios;
            HORAS_VALIDAS = data.horarios.map(h => parseInt(h.split(':')[0])); // <--- CRÍTICO: Actualizar la variable global
        }
        ...
        console.log(`✅ HORAS_VALIDAS: ${HORAS_VALIDAS.join(', ')}`);
...
app.post('/turnos', async (req, res) => {
    ...
    if (!diaLower || !DIAS_VALIDOS.includes(diaLower)) {
        console.log(`❌ Día inválido: ${diaLower}. Días válidos: ${DIAS_VALIDOS.join(', ')}`);
    ...
    if (isNaN(hn) || !HORAS_VALIDAS.includes(hn)) {
        console.log(`❌ Hora inválida: ${hora}. Horas válidas: ${HORAS_VALIDAS.join(', ')}`);
        return res.status(400).json({ error: `Hora inválida. Horarios disponibles: ${HORAS_VALIDAS.join(', ')}` });
    }
    ...
    if (turnos.some(t => t.dia === diaLower && t.hora === hn)) {
        // Buscar alternativa
        const alternativa = buscarAlternativa(turnos, diaLower);
        if (alternativa) {
    ...
    console.log(`✅ Turno reservado: ${nuevoTurno.nombre} - ${diaLower} ${hn}:00`);
...
// Función para buscar alternativa de horario
function buscarAlternativa(turnos, dia) {
    const horariosDisponibles = HORAS_VALIDAS.filter(h => 
        !turnos.some(t => t.dia === dia && t.hora === h)
    );
    ...
    // Buscar en el día siguiente
    const diasSemana = ['domingo', 'lunes', 'martes', 'miercoles', 'jueves', 'viernes', 'sabado'];
    const idxActual = diasSemana.indexOf(dia);
    for (let i = 1; i <= 3; i++) {
        const idxSiguiente = (idxActual + i) % 7;
        const diaSiguiente = diasSemana[idxSiguiente];
        if (DIAS_VALIDOS.includes(diaSiguiente)) {
            const horariosLibres = HORAS_VALIDAS.filter(h => 
                !turnos.some(t => t.dia === diaSiguiente && t.hora === h)
            );
