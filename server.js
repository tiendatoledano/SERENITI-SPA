...
// ============================================================
// WHATSAPP - ENVIAR MENSAJE DE CONFIRMACIÓN
// ============================================================
app.post('/api/enviar-whatsapp', async (req, res) => {
    try {
        const { telefono, codigoPais, nombre, massageType, dia, hora, fecha, ubicacion, codigoCancelacion } = req.body;
        
        console.log('📤 Preparando mensaje WhatsApp para:', nombre);
        
        if (!telefono) {
            return res.status(400).json({ error: 'Teléfono requerido' });
        }
        
        const mensaje = `🌿 *SERENITY SPA*\n\nHola *${nombre}*, ¡Gracias por confiar en nosotros! ✨\n\n✅ *RESERVA CONFIRMADA*\n\n📅 *Día:* ${dia}\n📆 *Fecha:* ${fecha || dia}\n⏰ *Hora:* ${hora}:00 hs\n💆‍♂️ *Masaje:* ${massageType || 'Masaje'}\n📍 ${ubicacion || 'Salón Serenity Spa'}\n\n🔑 *Código de cancelación:* ${codigoCancelacion || 'N/A'}\n\n🌸 Te esperamos.\n⏱️ Cancelá con 4hs de anticipación.\n\n*Equipo Serenity Spa*`;
        
        const numero = `${codigoPais || '53'}${telefono.replace(/\D/g, '')}`;
