...
app.use((req, res, next) => {
    ...
    const rutasPermitidas = [
        ...
        '/api/config/registro', '/api/servicios', '/api/upload-hero', 
        '/api/ia', '/api/recargar', '/api/enviar-whatsapp',
...
// ============================================================
// CONFIGURACIÓN REGISTRO - PUT (CORREGIDO)
// ============================================================
...
app.put('/api/config/registro', async (req, res) => {
    ...
    try {
        ...
        if (!data.hero || !data.beneficios || !data.conceptos) {
            return res.status(400).json({ 
                success: false, 
                error: 'Datos incompletos. Se requiere hero, beneficios y conceptos.' 
            });
        }
        
        // Validar que beneficios tenga 6 elementos
        if (!Array.isArray(data.beneficios) || data.beneficios.length < 6) {
            return res.status(400).json({
                success: false,
                error: 'Se requieren 6 beneficios.'
            });
        }
        
        // Validar que conceptos tenga 6 elementos
        if (!Array.isArray(data.conceptos) || data.conceptos.length < 6) {
            return res.status(400).json({
                success: false,
                error: 'Se requieren 6 conceptos.'
            });
        }
        
        await fs.writeFile(REGISTRO_FILE, JSON.stringify(data, null, 2));
        console.log('✅ Configuración de registro guardada correctamente');
        console.log('📋 Hero:', data.hero);
        console.log('📋 Beneficios:', data.beneficios.length);
        console.log('📋 Conceptos:', data.conceptos.length);
