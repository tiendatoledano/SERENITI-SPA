    ...
    try {
        ...
        try {
            ...
            if (servicio && servicio.imagen_whatsapp) {
                ...
                // ✅ CONVERTIR URL DE POSTIMAGES A URL DIRECTA
                // Si es postimg.cc/xxxx, convertir a i.postimg.cc/xxxx.jpg
                if (imgUrl.includes('postimg.cc')) {
                    // Si no tiene i.postimg.cc, convertir
                    if (!imgUrl.includes('i.postimg.cc')) {
                        const match = imgUrl.match(/postimg\.cc\/([a-zA-Z0-9]+)/);
                        if (match) {
                            const code = match[1];
                            imgUrl = `https://i.postimg.cc/${code}.jpg`;
                            console.log(`📸 Convertida URL de Postimages: ${imgUrl}`);
                        }
                    }
                }
                
                // ✅ Si es una URL de Postimages pero sin extensión, agregar .jpg
                if (imgUrl.includes('i.postimg.cc') && !imgUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
                    imgUrl = imgUrl + '.jpg';
                    console.log(`📸 Agregada extensión .jpg: ${imgUrl}`);
                }
                
                imagenWhatsApp = imgUrl;
                console.log(`📸 Imagen WhatsApp final: ${imagenWhatsApp}`);
            }
        } catch (e) {
...
// ============================================================
app.ost('/api/seguridad/bloquear-usuario/:id', verifyAuth, async (req, res) => {
    try {
        const id = req.params.id;
