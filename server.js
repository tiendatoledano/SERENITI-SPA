...
// ============================================================
// WHATSAPP - ENVÍO DE MENSAJE CON IMAGEN (CORREGIDO)
// ============================================================
    ...
    try {
        ...
        try {
            ...
            if (servicio && servicio.imagen_whatsapp) {
                ...
                console.log(`📸 URL de WhatsApp del servicio: ${imagenWhatsApp}`);
            }
        } catch (e) {
...
// ============================================================
// MIGRACIÓN DE DATOS - CORREGIDA
// ============================================================
async function migrarServiciosPorDefecto() {
    ...
    for (const s of serviciosPorDefecto) {
        try {
            ...
            console.warn(`⚠️ Error creando servicio ${s.nombre}:`, e.message);
        }
    }
...
async function migrarDatos() {
    ...
    try {
        ...
        if (servicios.length === 0) {
            console.log('📦 No hay servicios en Supabase, verificando JSON...');
            try {
                const filePath = path.join(BASE_DIR, 'servicios.json');
                await fs.access(filePath);
                const serviciosData = JSON.parse(await fs.readFile(filePath, 'utf8'));
                ...
                console.log(`✅ ${serviciosData.length} servicios migrados desde JSON`);
            } catch (e) {
                console.log('ℹ️ Archivo servicios.json no encontrado o vacío, usando datos por defecto');
                await migrarServiciosPorDefecto();
            }
        }
        ...
        if (testimonios.length === 0) {
            console.log('📦 No hay testimonios en Supabase, verificando JSON...');
            try {
                const filePath = path.join(BASE_DIR, 'testimonios.json');
                await fs.access(filePath);
                const testimoniosData = JSON.parse(await fs.readFile(filePath, 'utf8'));
                ...
                console.log(`✅ ${testimoniosData.length} testimonios migrados desde JSON`);
            } catch (e) {
                console.log('ℹ️ Archivo testimonios.json no encontrado o vacío');
            }
        }
