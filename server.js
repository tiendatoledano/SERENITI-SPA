...
// ============================================================
// MIDDLEWARE DE BLOQUEO DE IP - PANTALLA EN BLANCO
// ============================================================

// Ruta para la página de bloqueo
...
app.use(async (req, res, next) => {
    ...
    if (req.path.endsWith('.css') || req.path.endsWith('.js') || req.path.endsWith('.png') || req.path.endsWith('.jpg') || req.path.endsWith('.svg') || req.path.endsWith('.ico') || req.path.endsWith('.webp')) {
        return next();
    }
...
app.post('/api/seguridad/bloquear-ip', verifyAuth, async (req, res) => {
    try {
        ...
        const resultado = await security.bloquearIP(
            ip, 
