...
function detectBaseDir() {
    const possiblePaths = [
        __dirname,                          // Raíz del proyecto
        path.join(__dirname, 'src'),        // Carpeta src/
        path.join(__dirname, 'public'),     // Carpeta public/
        path.join(__dirname, '..'),         // Un nivel arriba
        path.join(__dirname, 'dist'),       // Carpeta dist/
        path.join(__dirname, 'build'),      // Carpeta build/
    ];
    ...
    // Si no encuentra index.html, usar __dirname
...
// También servir desde la raíz original como fallback
app.use(express.static(__dirname));

// ============================================================
// RUTAS HTML EXPLÍCITAS - CORREGIDAS
// ============================================================
function sendFileSafe(fileName) {
    return (req, res) => {
        ...
        if (fs.existsSync(filePath)) {
            ...
            // Buscar en la raíz como fallback
...
// ============================================================
// ⭐ ADMIN - ELIMINAR TURNO (NUEVO ENDPOINT PARA ADMIN)
// ============================================================
app.delete('/api/admin/turnos/:id', verifyAuth, async (req, res) => {
    try {
        ...
        console.log(`🗑️ Admin eliminando turno: ${id} - Cliente: ${turno.nombre} - Servicio: ${turno.massage_type} - Estado: ${turno.estado}`);
