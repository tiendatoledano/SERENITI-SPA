...
// server.js - Función WhatsApp con imágenes optimizadas para vista ancha
app.post('/api/enviar-whatsapp/:id', verifyAuth, async (req, res) => {
    try {
        ...
        // ====== IMAGEN DEL SERVICIO OPTIMIZADA PARA VISTA ANCHA ======
        ...
        try {
            ...
            if (servicio && servicio.imagen_whatsapp) {
                // Si la imagen es de Postimages, redimensionar con parámetros
                let imgUrl = servicio.imagen_whatsapp;
                
                // Si es de Postimages, agregar parámetros de tamaño
                if (imgUrl.includes('postimg.cc')) {
                    // Convertir a URL directa con parámetros de tamaño
                    const directUrl = imgUrl.replace('postimg.cc', 'i.postimg.cc');
                    imagenWhatsApp = directUrl + '?w=1200&h=630&fit=crop';
                } else {
                    imagenWhatsApp = imgUrl;
                }
            }
        } catch (e) {
            console.warn('⚠️ Error obteniendo imagen del servicio:', e);
        }
