...
// ============================================================
// ARCHIVOS Y DIRECTORIOS (para fallback)
// ============================================================
...
app.use((req, res, next) => {
    ...
    const rutasPermitidas = [
        ...
        '/api/seguridad/bloquear-usuario', '/api/premium/usuario', '/api/supabase/registro'
    ];
...
// ============================================================
// CONFIGURACIÓN REGISTRO - CON SUPABASE
// ============================================================

const REGISTRO_TABLE = 'registro_config';
...
// Obtener configuración de registro desde archivo local (fallback)
...
// Guardar configuración de registro en Supabase
async function saveRegistroConfigToSupabase(data) {
    if (!supabase) {
        console.warn('⚠️ Supabase no disponible, guardando en archivo local');
        return false;
    }
    ...
    try {
        ...
        if (checkError && checkError.code !== 'PGRST116') {
            console.warn('⚠️ Error verificando registro existente:', checkError.message);
        }
        
        let result;
        if (existingData && existingData.id) {
            // Actualizar registro existente
            console.log('📝 Actualizando configuración de registro en Supabase...');
            ...
            console.log('📝 Creando nueva configuración de registro en Supabase...');
            result = await supabase
                .from(REGISTRO_TABLE)
                .insert({
                    hero: normalizedData.hero,
        ...
        if (result.error) {
            console.error('❌ Error guardando en Supabase:', result.error.message);
            return false;
        }
        
        console.log('✅ Configuración de registro guardada en Supabase correctamente');
        return true;
    } catch (e) {
...
// ============================================================
// RUTAS DE REGISTRO CON SUPABASE
// ============================================================
...
app.put('/api/config/registro', async (req, res) => {
    ...
    try {
        ...
        // Guardar en Supabase
        const savedToSupabase = await saveRegistroConfigToSupabase(normalizedData);
        
        // También guardar en archivo local como fallback
        try {
            await fs.writeFile(REGISTRO_FILE, JSON.stringify(normalizedData, null, 2));
            console.log('✅ Configuración guardada en archivo local');
        } catch (e) {
            console.warn('⚠️ Error guardando archivo local:', e.message);
        }
