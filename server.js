...
app.use(async (req, res, next) => {
    try {
        await security.seguridadMiddleware(req, res, next);
    } catch (e) {
        console.error('Error en middleware de seguridad:', e);
        next();
    }
});
...
// ============================================================
// SERVICIOS - CRUD COMPLETO CON IMÁGENES (CORREGIDO)
// ============================================================
app.get('/api/servicios', async (req, res) => {
    try {
        ...
        const serviciosFormateados = servicios.map(s => ({
            ...
            efectos: s.efectos || [],
            imagenWeb: s.imagen_web || s.imagenWeb || "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?w=800",
            imagenWhatsApp: s.imagen_whatsapp || s.imagenWhatsApp || '',
...
app.post('/api/servicios', verifyAuth, async (req, res) => {
    try {
        ...
        // Usar los nombres de campo correctos para Supabase
        const imagenWebFinal = imagenWeb || "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?w=800";
        const imagenWhatsAppFinal = imagenWhatsApp || '';
        ...
        // Devolver con nombres consistentes para el frontend
        res.status(201).json({
            id: nuevoServicio.id,
        ...
        res.status(500).json({ error: 'Error al crear el servicio: ' + e.message });
    }
});
...
app.put('/api/servicios/:id', verifyAuth, async (req, res) => {
    try {
        ...
        const imagenWebFinal = imagenWeb || servicioExistente.imagen_web || "https://images.unsplash.com/photo-1544161515-4ab6ce6db874?w=800";
        const imagenWhatsAppFinal = imagenWhatsApp || servicioExistente.imagen_whatsapp || '';
...
// ============================================================
// RUTA DE BLOQUEO DE USUARIO POR IP (desde admin)
// ============================================================
app.post('/api/seguridad/bloquear-usuario/:turnoId', verifyAuth, async (req, res) => {
    try {
        const turnoId = req.params.turnoId;
        const turno = await db.getTurnoById(turnoId);
        ...
        if (ipUsuario === 'IP desconocida' || ipUsuario === 'N/A' || ipUsuario === '0.0.0.0') {
            return res.status(400).json({ success: false, error: 'No se puede bloquear: IP desconocida' });
        }
        
        // Incrementar contador de bloqueos para este turno
        const nuevoConteo = (turno.conteo_bloqueos || 0) + 1;
        await db.updateTurno(turnoId, { conteo_bloqueos: nuevoConteo });
        
        // Bloquear la IP si ya tiene 3 o más bloqueos
        let mensaje = `Usuario ${turno.nombre} marcado para bloqueo (${nuevoConteo}ra vez)`;
        let bloqueado = false;
        
        if (nuevoConteo >= 3) {
            await security.bloquearIP(ipUsuario, `Usuario ${turno.nombre} - ${nuevoConteo} bloqueos`, 'USUARIO_BLOQUEADO', 72, '/api/seguridad/bloquear-usuario');
            await db.updateTurno(turnoId, { bloqueado: true });
            bloqueado = true;
            mensaje = `IP ${ipUsuario} bloqueada permanentemente (${nuevoConteo} bloqueos)`;
        }
        ...
        res.status(500).json({ success: false, error: 'Error al bloquear usuario: ' + error.message });
    }
});

// ============================================================
// RUTA PARA MARCAR WHATSAPP COMO ENVIADO
// ============================================================
app.post('/api/turnos/:id/whatsapp-enviado', verifyAuth, async (req, res) => {
    try {
        const id = req.params.id;
        await db.updateTurno(id, { confirmado_whatsapp: true });
        res.json({ success: true });
    } catch (error) {
        console.error('❌ Error marcando WhatsApp como enviado:', error);
