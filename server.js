// ============================================================
// server.js - SERVIDOR PRINCIPAL CON SUPABASE + MAGIC LINKS
// ============================================================
...
console.log('📌 FRONTEND_URL:', process.env.FRONTEND_URL || 'http://localhost:' + PORT);
...
app.use(async (req, res, next) => {
    const excludedPaths = ['/bloqueado', '/api/seguridad', '/api/verify', '/api/login', '/api/config-emailjs', '/api/usuario', '/auth/magic'];
...
// ============================================================
// CONFIGURACIÓN EMAIL PARA MAGIC LINKS
// ============================================================
...
app.post('/api/auth/magic-link', async (req, res) => {
    try {
        ...
        // 1. VERIFICAR BLOQUEO DE IP
        const bloqueo = await verificarBloqueoIP(ip);
        if (bloqueo.bloqueado) {
            return res.status(429).json({ error: bloqueo.error });
        }
        
        // 2. VERIFICAR LÍMITE DE SOLICITUDES
        const limite = await verificarLimiteSolicitudes(emailLower, ip);
        if (!limite.permitido) {
        ...
        await db.createMagicLink(emailLower, token, expiracion.toISOString(), nombre);
        
        // 5. GENERAR URL DEL ENLACE MÁGICO
        const baseUrl = process.env.FRONTEND_URL || `http://localhost:${PORT}`;
        const magicUrl = `${baseUrl}/auth/magic?token=${token}&email=${encodeURIComponent(emailLower)}`;
        
        // 6. ENVIAR POR EMAIL
        const nombreUsuario = nombre || emailLower.split('@')[0];
        const enviado = await enviarMagicLinkEmail(emailLower, nombreUsuario, magicUrl);
        
        if (!enviado) {
            return res.status(500).json({ error: 'Error al enviar el email. Verifica la configuración.' });
        }
        
        console.log(`📧 Enlace mágico enviado a ${emailLower}`);
        
        res.json({
            success: true,
            mensaje: 'Revisa tu correo para acceder',
            email: emailLower
        });
...
// ============================================================
// 2. VERIFICAR ENLACE MÁGICO (CALLBACK)
// ============================================================
...
app.get('/auth/magic', async (req, res) => {
    try {
        ...
        if (!token || !email) {
            return res.status(400).send(`
                ...
                <head>
                    <meta charset="UTF-8">
                    <meta name="viewport" content="width=device-width, initial-scale=1.0">
                    <title>Enlace inválido - Serenity Spa</title>
                    <style>
                        body { font-family: 'Lato', sans-serif; background: #0a0a0a; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 1rem; color: #fff; }
                        .container { text-align: center; max-width: 400px; padding: 2rem; border: 1px solid rgba(201, 168, 122, 0.2); border-radius: 16px; background: rgba(20, 10, 5, 0.95); }
                        .icon { font-size: 4rem; color: #dc2626; }
                        h1 { color: #dc2626; font-family: 'Cinzel', serif; }
                        p { color: rgba(255,255,255,0.7); }
                        a { color: #c9a87a; text-decoration: underline; }
                    </style>
                </head>
                <body>
                    <div class="container">
                        ...
                        <p>Faltan parámetros necesarios para la autenticación.</p>
            ...
            return res.status(400).send(`
                ...
                <head>
                    ...
                    <style>
                        ...
                        p { color: rgba(255,255,255,0.7); }
                        a { color: #c9a87a; text-decoration: underline; }
                        .btn { display: inline-block; margin-top: 1rem; padding: 0.6rem 1.5rem; background: #c9a87a; color: #0a0a0a; border-radius: 50px; text-decoration: none; font-weight: 700; }
                    </style>
                </head>
                <body>
                    <div class="container">
                        ...
                        <p>El enlace que intentas usar no existe.</p>
        ...
        if (magicLink.email !== email) {
            return res.status(400).send(`
                ...
                <head>
                    <meta charset="UTF-8">
                    <meta name="viewport" content="width=device-width, initial-scale=1.0">
                    <title>Error - Serenity Spa</title>
                    <style>
                        body { font-family: 'Lato', sans-serif; background: #0a0a0a; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 1rem; color: #fff; }
                        .container { text-align: center; max-width: 400px; padding: 2rem; border: 1px solid rgba(201, 168, 122, 0.2); border-radius: 16px; background: rgba(20, 10, 5, 0.95); }
                        .icon { font-size: 4rem; color: #dc2626; }
                ...
                <body>
                    <div class="container">
                        ...
                        <p>El enlace no corresponde a este email.</p>
                        <p><a href="/">Volver al inicio</a></p>
                    </div>
                </body>
                </html>
        ...
        if (magicLink.usado) {
            return res.status(400).send(`
                ...
                <head>
                    <meta charset="UTF-8">
                    <meta name="viewport" content="width=device-width, initial-scale=1.0">
                    <title>Enlace usado - Serenity Spa</title>
                    <style>
                        body { font-family: 'Lato', sans-serif; background: #0a0a0a; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 1rem; color: #fff; }
                        .container { text-align: center; max-width: 400px; padding: 2rem; border: 1px solid rgba(201, 168, 122, 0.2); border-radius: 16px; background: rgba(20, 10, 5, 0.95); }
                        .icon { font-size: 4rem; color: #f59e0b; }
                        h1 { color: #f59e0b; font-family: 'Cinzel', serif; }
                        p { color: rgba(255,255,255,0.7); }
                        a { color: #c9a87a; text-decoration: underline; }
                        .btn { display: inline-block; margin-top: 1rem; padding: 0.6rem 1.5rem; background: #c9a87a; color: #0a0a0a; border-radius: 50px; text-decoration: none; font-weight: 700; }
                    </style>
                ...
                <body>
                    <div class="container">
                        <div class="icon">⏳</div>
                        <h1>Enlace ya usado</h1>
                        <p>Este enlace ya fue utilizado. Solicita uno nuevo.</p>
                        <p><a href="/" class="btn">Solicitar nuevo enlace</a></p>
                    </div>
                </body>
                </html>
            `);
        ...
        if (new Date(magicLink.expiracion) < new Date()) {
            return res.status(400).send(`
                <!DOCTYPE html>
                <html>
                <head>
                    <meta charset="UTF-8">
                    <meta name="viewport" content="width=device-width, initial-scale=1.0">
                    <title>Enlace expirado - Serenity Spa</title>
                    <style>
                        body { font-family: 'Lato', sans-serif; background: #0a0a0a; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; padding: 1rem; color: #fff; }
                        .container { text-align: center; max-width: 400px; padding: 2rem; border: 1px solid rgba(201, 168, 122, 0.2); border-radius: 16px; background: rgba(20, 10, 5, 0.95); }
                        .icon { font-size: 4rem; color: #f59e0b; }
        ...
        // 5. ✅ ENLACE VÁLIDO - AUTENTICAR USUARIO
        
        // Marcar como usado
        await db.markMagicLinkAsUsed(token);
        
        // Buscar o crear usuario
        let usuario = await db.getUsuarioByEmail(email);
        
        if (!usuario) {
            usuario = {
                id: generarId(),
                nombre: magicLink.nombre || email.split('@')[0],
                email: email,
                verificado: true,
        ...
        await db.createTokenUsuario(sessionToken, email);
        
        // Redirigir al frontend con el token
        const frontendUrl = process.env.FRONTEND_URL || '/';
        res.redirect(`${frontendUrl}?auth=success&token=${sessionToken}&nombre=${encodeURIComponent(usuario.nombre)}`);
        
    } catch (error) {
        console.error('❌ Error en callback magic link:', error);
        res.status(500).send(`
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                <div class="container">
                    <div class="icon">❌</div>
                    <h1>Error de autenticación</h1>
                    <p>${escapeHtml(error.message || 'Error interno del servidor')}</p>
                    <p><a href="/">Volver al inicio</a></p>
                </div>
            </body>
            </html>
...
// ============================================================
// 3. FUNCIÓN PARA ENVIAR EMAIL DE MAGIC LINK
// ============================================================

async function enviarMagicLinkEmail(email, nombre, magicUrl) {
    try {
        const mailOptions = {
            from: `"Serenity Spa" <${process.env.EMAIL_USER}>`,
            to: email,
            subject: '🔑 Tu acceso a Serenity Spa',
            html: `
                <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background: #f8f3ec; border-radius: 16px; border: 1px solid rgba(201, 168, 122, 0.2);">
                    
                    <div style="text-align: center; padding: 20px 0 10px 0; border-bottom: 1px solid rgba(201, 168, 122, 0.2);">
                        <h1 style="color: #c9a87a; font-family: 'Cinzel', serif; font-size: 28px; margin: 0;">
                            🌿 Serenity Spa
                        </h1>
                        <p style="color: #5a4a3a; font-size: 14px; margin: 5px 0 0 0;">Bienestar que se siente, donde estés</p>
                    </div>
                    
                    <div style="padding: 30px 20px;">
                        <p style="color: #333; font-size: 18px; line-height: 1.6; margin: 0 0 20px 0;">
                            Hola <strong style="color: #c9a87a;">${escapeHtml(nombre)}</strong>!
                        </p>
                        
                        <p style="color: #555; font-size: 16px; line-height: 1.6; margin: 0 0 25px 0;">
                            Haz clic en el botón para acceder a tu cuenta. 
                            <strong style="color: #c9a87a;">Un solo clic</strong> y estarás dentro.
                        </p>
                        ...
                        <p style="color: #888; font-size: 13px; line-height: 1.6; text-align: center; margin: 0 0 10px 0;">
                            ⏱️ Este enlace es <strong>válido por 15 minutos</strong> y de <strong>un solo uso</strong>.
                        </p>
                        
                        <div style="background: #f5f0ea; border-radius: 8px; padding: 15px; margin: 20px 0; border-left: 4px solid #c9a87a;">
                            <p style="color: #666; font-size: 13px; margin: 0;">
                                <span style="font-weight:600;">💡 ¿No solicitaste este enlace?</span><br>
                                Ignora este mensaje. Alguien pudo haber ingresado tu email por error.
                            </p>
                        </div>
                    </div>
                    
                    <div style="text-align: center; padding: 20px 0 10px 0; border-top: 1px solid rgba(201, 168, 122, 0.2);">
                        <p style="color: #999; font-size: 12px; margin: 0;">
                            © 2026 Serenity Spa - Todos los derechos reservados
                        </p>
                    </div>
                </div>
            `
...
// ============================================================
// 4. FUNCIONES ANTI-ABUSO
// ============================================================

const ANTI_ABUSO = {
    INTENTOS_MAXIMOS: 5,
    SOLICITUDES_POR_HORA: 3,
    BLOQUEO_TEMPORAL: 15,
    BLOQUEO_IP_INTENTOS: 20
};

async function verificarLimiteSolicitudes(email, ip) {
    try {
        ...
        const { count, error } = await supabase
            ...
            .gte('fecha_creacion', horaAtras.toISOString());
        ...
        if (count >= ANTI_ABUSO.SOLICITUDES_POR_HORA) {
            return {
                permitido: false,
                error: `Has solicitado demasiados enlaces. Espera ${ANTI_ABUSO.SOLICITUDES_POR_HORA} hora.`
            };
        }
        
        return { permitido: true };
    } catch (e) {
        console.error('Error en verificarLimiteSolicitudes:', e);
        return { permitido: true };
    }
}
...
async function verificarBloqueoIP(ip) {
    try {
        if (!ip) return { bloqueado: false };
        
        const tiempoAtras = new Date(Date.now() - ANTI_ABUSO.BLOQUEO_TEMPORAL * 60 * 1000);
        
        const { count, error } = await supabase
            .from('intentos_login')
            .select('*', { count: 'exact', head: true })
            .eq('ip', ip)
            .gte('fecha', tiempoAtras.toISOString());
        
        if (error) {
            console.error('Error verificando bloqueo IP:', error);
            return { bloqueado: false };
        }
        
        if (count >= ANTI_ABUSO.BLOQUEO_IP_INTENTOS) {
            return {
                bloqueado: true,
                error: `Demasiados intentos fallidos. IP bloqueada por ${ANTI_ABUSO.BLOQUEO_TEMPORAL} minutos.`
            };
        }
        
        return { bloqueado: false };
    } catch (e) {
        console.error('Error en verificarBloqueoIP:', e);
        return { bloqueado: false };
    }
}

// ============================================================
// REGISTRO DE USUARIOS (MANTENIDO PARA COMPATIBILIDAD)
// ============================================================
...
app.delete('/api/admin/turnos/:id', verifyAuth, async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            turnoId: id,
            nombre: turno.nombre,
            massageType: turno.massage_type
        });
...
app.delete('/turnos/:id', verifyAuth, async (req, res) => {
    try {
        ...
        console.log(`🗑️ Admin eliminando turno (legacy): ${id} - Cliente: ${turno.nombre}`);
        await db.deleteTurno(id);
