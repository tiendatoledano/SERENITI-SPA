// ============================================================
// server.js - SERVIDOR PRINCIPAL CON SUPABASE Y GOOGLE AUTH
// ============================================================
...
app.use(async (req, res, next) => {
    const excludedPaths = ['/bloqueado', '/api/seguridad', '/api/verify', '/api/login', '/api/config-emailjs', '/api/usuario', '/auth/google', '/auth/google/callback', '/api/auth'];
...
// ============================================================
// CONFIGURACIÓN EMAILJS (MANTENIDO PARA OTROS USOS)
// ============================================================
...
/**
 * Inicio de autenticación con Google
 */
...
app.get('/auth/google/callback', async (req, res) => {
    try {
        ...
        const result = await googleAuth.verifyAuthCode(code, state);
        
        // Guardar token en la sesión
        req.session.userToken = result.token;
        req.session.userData = result.usuario;
        ...
        res.status(500).send(`
            ...
            <head>
                ...
                <style>
                    ...
                    .error-container { text-align: center; padding: 2rem; border: 1px solid rgba(220,38,38,0.3); border-radius: 12px; background: rgba(20,10,5,0.95); }
                    h1 { color: #dc2626; }
            ...
            <body>
                <div class="error-container">
                    ...
                    <p>${escapeHtml(error.message || 'Ocurrió un error al iniciar sesión con Google')}</p>
...
/**
 * Obtener información del usuario autenticado con Google
 */
app.get('/api/auth/google/user', (req, res) => {
    try {
        const token = req.session.userToken || req.headers.authorization?.substring(7);
        
        if (!token) {
            return res.status(401).json({ error: 'No autorizado' });
        }
        
        const userData = googleAuth.verifySession(token);
        ...
        console.error('❌ Error en /api/auth/google/user:', error);
...
app.post('/api/auth/google/logout', (req, res) => {
    try {
        ...
        if (token) {
            googleAuth.revokeSession(token);
        }
 ...
 */
app.get('/api/auth/google/verify', (req, res) => {
    try {
        const token = req.session.userToken || req.headers.authorization?.substring(7);
 ...
 */
app.post('/api/auth/telefono/registro', async (req, res) => {
    try {
        const { telefono, nombre, codigoPais } = req.body;
        ...
        // Guardar o actualizar código de verificación
        ...
        if (upsertError) {
            ...
            return res.status(500).json({ error: 'Error al guardar código de verificación' });
        }
        
        // Enviar código por SMS (usando Twilio o similar)
        // Por ahora, devolvemos el código para desarrollo
        // En producción, enviar SMS real
        
        console.log(`📱 Código para ${telefonoLimpio}: ${codigo}`);
        ...
        console.error('❌ Error en registro telefónico:', error);
 ...
 */
app.post('/api/auth/telefono/verificar', async (req, res) => {
    try {
        ...
        if (registro.codigo !== codigo) {
            ...
            return res.status(400).json({ 
                error: `Código incorrecto. Te quedan ${5 - (registro.intentos || 0) - 1} intentos.` 
            });
        }
        ...
        const userData = {
            ...
            telefono: telefonoLimpio,
            codigo_pais: registro.codigo_pais || '53',
            verificado: true,
 ...
 */
app.post('/api/auth/telefono/login', async (req, res) => {
    try {
        ...
        if (error || !usuario) {
            return res.status(404).json({ 
                error: 'Este número no está registrado. Regístrate primero.' 
            });
        }
        ...
        const { error: upsertError } = await supabase
            ...
            .upsert({
                ...
                codigo_pais: usuario.codigo_pais || '53',
                intentos: 0,
                created_at: new Date().toISOString()
            }, { onConflict: 'telefono' });
        ...
        console.log(`📱 Código de login para ${telefonoLimpio}: ${codigo}`);
        ...
        console.error('❌ Error en login telefónico:', error);
        res.status(500).json({ error: 'Error interno del servidor' });
    }
});

// ============================================================
// AUTENTICACIÓN DE USUARIOS (LEGACY - MANTENIDO PARA COMPATIBILIDAD)
// ============================================================
...
app.get('/api/usuario/actual', async (req, res) => {
    ...
    const token = h.substring(7);
    try {
        // Primero verificar sesión de Google
        const googleUser = googleAuth.verifySession(token);
        if (googleUser) {
            return res.json({
                id: googleUser.id,
        ...
        // Si no es Google, buscar en Supabase
    ...
    try {
        ...
        if (authHeader && authHeader.startsWith('Bearer ')) {
            ...
            try {
                ...
                const googleUser = googleAuth.verifySession(token);
                if (googleUser) {
                    usuarioId = googleUser.id;
                    usuarioNombre = googleUser.nombre;
                } else {
                    // Verificar Supabase
                    const usuario = await db.getUsuarioByToken(token);
                    if (usuario) {
                ...
                if (usuarioNombre) {
                    console.log(`✅ Turno asociado al usuario: ${usuarioNombre} (${usuarioId})`);
                }
            } catch (e) {
...
app.get('/api/turnos/mis-turnos', async (req, res) => {
    ...
    try {
        ...
        if (googleUser) {
            ...
            usuario = await db.getUsuarioByToken(token);
            if (usuario) {
                usuarioId = usuario.id;
                usuarioNombre = usuario.nombre;
            }
        ...
        console.log(`🔍 Buscando turnos para usuario: ${usuarioNombre} (${usuarioId})`);
...
app.post('/api/turnos/confirmar/:id', async (req, res) => {
    ...
    try {
        ...
        if (turno.usuario_id && turno.usuario_id !== usuarioId) {
            return res.status(403).json({ error: 'No tienes permiso para confirmar este turno' });
        }
        if (!turno.usuario_id && turno.nombre !== usuarioNombre) {
            return res.status(403).json({ error: 'No tienes permiso para confirmar este turno' });
        }
