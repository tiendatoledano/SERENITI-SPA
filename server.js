...
// ============================================================
// CONFIGURACIÓN RESEND (API HTTP, SIN SMTP)
// ============================================================
...
async function initAllFiles() {
    ...
    // TOKEN-EMAIL.json
    if (!fsSync.existsSync(TOKEN_EMAIL_FILE)) {
        await fs.writeFile(TOKEN_EMAIL_FILE, JSON.stringify({}, null, 2));
        console.log('✅ token-email.json creado');
    }
...
async function getUsuarioByToken(token) {
    try {
        ...
        const email = tokenEmailData[token];
        ...
        console.error('Error obteniendo usuario por token:', e);
        return null;
    }
}
...
// ============================================================
// RUTAS DE REGISTRO Y VERIFICACIÓN DE USUARIOS (CON RESEND)
// ============================================================
app.post('/api/registro/solicitar-codigo', async (req, res) => {
    try {
        ...
        const { data, error } = await resend.emails.send({
            from: 'Serenity Spa <onboarding@resend.dev>',
            to: [email],
        ...
        if (error) {
            ...
            return res.status(500).json({ error: 'Error al enviar el correo. Por favor, intenta de nuevo.' });
        }
...
app.post('/api/registro/verificar-codigo', async (req, res) => {
    try {
        ...
        // Verificar si ya existe (actualizar si es necesario)
...
app.put('/api/usuario/perfil', async (req, res) => {
    try {
        ...
        if (avatar) {
            usuarios[usuarioIndex].avatar = avatar;
        }
        ...
        res.json({ 
            ...
            usuario: {
                ...
                avatar: usuarios[usuarioIndex].avatar || null
            }
        });
    } catch (error) {
...
// ============================================================
// RUTA PARA GUARDAR TESTIMONIOS - CORREGIDA (MÁS ROBUSTA)
// ============================================================
app.post('/api/testimonios', async (req, res) => {
    try {
        const { nombre, calificacion, comentario, imagen, publico } = req.body;
        
        console.log('📝 Recibiendo testimonio:', { nombre, calificacion, comentario });
        ...
        console.log(`✅ Testimonio guardado de ${testimonio.nombre} (${testimonio.calificacion}⭐) - Público: ${testimonio.publico}`);
...
// ============================================================
// RUTAS PÚBLICAS DE TESTIMONIOS - CORREGIDAS
// ============================================================
app.get('/api/testimonios', async (req, res) => {
    try {
        ...
        const publicos = testimonios
            .filter(t => t.publico !== false) // Si no tiene publico: false, se muestra
...
app.post('/api/testimonios', async (req, res) => {
    try {
        ...
        try {
            ...
            // El archivo no existe o está corrupto, crear nuevo
            console.log('📝 Creando nuevo archivo de testimonios');
            testimonios = [];
        }
        ...
        console.log(`✅ Nuevo testimonio guardado de ${testimonio.nombre} (${testimonio.calificacion}⭐)`);
        res.status(201).json({ 
            success: true, 
            testimonio: testimonio,
            message: 'Testimonio guardado exitosamente'
...
// ============================================================
// VERIFICACIÓN DE TOKEN DE USUARIO - MEJORADA
// ============================================================
app.get('/api/verify/user', (req, res) => {
    const h = req.headers.authorization;
    if (!h || !h.startsWith('Bearer ')) {
        return res.json({ valid: false, error: 'No token' });
    }
    const token = h.substring(7);
    
    // Verificar en el mapa de tokens
    if (validTokens.has(token) && validTokens.get(token) > Date.now()) {
        return res.json({ valid: true });
    }
    
    // Verificar en el archivo de token-email
    try {
        const tokenEmailData = JSON.parse(fsSync.readFileSync(TOKEN_EMAIL_FILE, 'utf8'));
        if (tokenEmailData[token]) {
            // El token existe, verificar si el usuario sigue activo
            return res.json({ valid: true });
        }
    } catch (e) {
...
app.get('/api/ia/personalidad', async (req, res) => {
    try {
        // Leer la configuración principal
        const data = JSON.parse(await fs.readFile(CONFIG_FILE, 'utf8'));
        // Si existe una sección de IA en la configuración, usarla
