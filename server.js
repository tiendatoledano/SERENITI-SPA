// ============================================================
// server.js - SERVIDOR PRINCIPAL CON SUPABASE (OPTIMIZADO)
// ============================================================
...
// ============================================================
// ⭐ CACHÉ EN MEMORIA PARA BLOQUEOS (REDUCE LECTURAS DE DISCO)
// ============================================================
let bloqueosCache = {
    data: [],           // Lista de IPs bloqueadas
    timestamp: 0,       // Momento de la última actualización
    ttl: 5000          // 5 segundos de validez (balance entre velocidad y actualización)
};

async function getBloqueosCache() {
    const ahora = Date.now();
    // Si el caché es válido, devolverlo
    if (bloqueosCache.data.length > 0 && (ahora - bloqueosCache.timestamp) < bloqueosCache.ttl) {
        return bloqueosCache.data;
    }
    
    // Si no, actualizar desde el disco
    try {
        const bloqueos = await security.getBloqueosActivos();
        bloqueosCache.data = bloqueos.activos || [];
        bloqueosCache.timestamp = ahora;
        return bloqueosCache.data;
    } catch (error) {
        console.error('❌ Error actualizando caché de bloqueos:', error);
        return bloqueosCache.data || [];
    }
}

// ============================================================
// FUNCIÓN RÁPIDA PARA VERIFICAR BLOQUEO (CON CACHÉ)
// ============================================================
async function isIPBloqueada(ip) {
    // IPs locales nunca se bloquean (rápido)
    if (ip === '127.0.0.1' || ip === '::1' || ip.startsWith('192.168.') || ip.startsWith('10.') || ip.startsWith('169.254.')) {
        return false;
    }
    
    // Obtener del caché (rápido)
    const bloqueos = await getBloqueosCache();
    const bloqueado = bloqueos.some(b => b.ip === ip && !b.expirado);
    
    if (bloqueado) {
        console.log(`🛡️ IP BLOQUEADA: ${ip}`);
    }
    return bloqueado;
}
...
// ============================================================
// INICIALIZAR SEGURIDAD (ANTES DE CUALQUIER COSA)
// ============================================================
(async () => {
    await security.initSecurityFiles();
    // Precargar caché de bloqueos
    await getBloqueosCache();
    console.log('🛡️ Sistema de seguridad blindado iniciado');
})();

// ============================================================
// ⭐ MIDDLEWARE DE BLOQUEO - PRIMERO EN EJECUTARSE (OPTIMIZADO)
// ============================================================
app.use(async (req, res, next) => {
    // Obtener IP del cliente (rápido)
    const ip = security.getClientIP(req);
    
    // ⭐ RUTAS EXCLUIDAS - NUNCA SE BLOQUEAN
    const excludedPaths = [
        '/bloqueado',
        '/api/seguridad',
        '/api/verify',
    ...
    // ⭐ VERIFICAR BLOQUEO (USANDO CACHÉ - RÁPIDO)
    const bloqueado = await isIPBloqueada(ip);
    
    if (bloqueado) {
        console.log(`🚫 ACCESO DENEGADO: ${ip} - ${req.path}`);
        ...
        <body>
            <div class="bloqueo-container">
                ...
                <p class="bloqueo-mensaje">
                    Su acceso ha sido cancelado en nuestro sistema por violación de los términos de uso.
                </p>
...
// ============================================================
// ⭐ SERVIR ARCHIVOS ESTÁTICOS - DESPUÉS DEL MIDDLEWARE DE BLOQUEO
// ============================================================
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (intentos > 5) {
            ...
            // Limpiar caché después de bloquear
            bloqueosCache.data = [];
            bloqueosCache.timestamp = 0;
...
// ============================================================
// REGISTRO DE USUARIOS (RESTO DEL CÓDIGO IGUAL)
// ============================================================
...
app.post('/api/enviar-whatsapp/:id', verifyAuth, async (req, res) => {
    try {
        ...
        try {
            ...
            if (servicio) {
                imagenWhatsApp = servicio.imagen_whatsapp || servicio.imagen_web || '';
            }
        } catch (e) {}
