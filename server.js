// ============================================================
// server.js - SERVIDOR PRINCIPAL CON SUPABASE (VERSIÓN FINAL CON GOOGLE AUTH)
// ============================================================
...
app.use(async (req, res, next) => {
    const excludedPaths = ['/bloqueado', '/api/seguridad', '/api/verify', '/api/login', '/api/config-emailjs', '/api/usuario', '/api/auth/google'];
...
app.get('/api/auth/google/callback', async (req, res) => {
    try {
        ...
        let usuarioExistente = await db.getUsuarioByEmail(user.email);
        ...
        if (!usuarioExistente) {
            ...
            if (avatar && usuarioExistente.avatar !== avatar) {
                await db.updateUsuario(usuarioExistente.id, { avatar });
            }
            // Asegurar que el usuario está verificado
            if (!usuarioExistente.verificado) {
                await db.updateUsuario(usuarioExistente.id, { verificado: true });
            }
        }

        // Generar nuestro propio token de sesión
        const token = crypto.randomBytes(64).toString('hex');
        await db.createTokenUsuario(token, user.email);

        // Crear HTML de cierre con los datos para enviar al frontend
        const html = `
            <!DOCTYPE html>
            <html>
            <head>
                ...
                <style>
                    ...
                    .spinner {
                        ...
                        width: 50px;
                        height: 50px;
                        border: 3px solid rgba(201, 168, 122, 0.1);
                    ...
                    h2 { color: #c9a87a; margin-bottom: 0.5rem; }
                    p { color: #a8906e; }
                </style>
            </head>
            <body>
                <div class="container">
                    ...
                    <p>Redirigiendo a Serenity Spa...</p>
                    <script>
                        const userData = {
                            token: '${token}',
                            nombre: '${escapeHtml(usuarioExistente.nombre || '')}',
                            email: '${escapeHtml(usuarioExistente.email || '')}',
                            avatar: '${escapeHtml(usuarioExistente.avatar || '')}'
                        };
...
// Ruta para cerrar sesión con Google
...
// ============================================================
// ELIMINAR TURNO - LEGACY (MANTENIDO POR COMPATIBILIDAD)
// ============================================================
...
app.get('/api/turnos/mis-turnos', async (req, res) => {
    ...
    try {
        ...
        // Buscar por usuario_id (más preciso)
        let turnosData = [];
