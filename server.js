...
const REGISTRO_FILE = path.join(__dirname, 'registro-config.json');
const INFRACCIONES_FILE = path.join(__dirname, 'infracciones.json');
...
async function registrarInfraccion(ip, tipo, detalle) {
    ...
    // Limpiar infracciones viejas (más de 24 horas)
    ...
    // Si tiene 3 infracciones en 24 horas, bloquear
    if (data.count >= 3) {
...
app.post('/turnos', async (req, res) => {
    ...
    // 1. Validar nombre (palabras prohibidas)
    if (nombre) {
        const palabra = contienePalabraProhibida(nombre);
        if (palabra) {
            const resultado = await registrarInfraccion(ip, 'palabra_prohibida_nombre', palabra);
            ...
            return res.status(400).json({ 
                error: `El nombre contiene la palabra prohibida: "${palabra}"`,
                advertencia: true,
    ...
    if (tel.length > 15) {
        return res.status(400).json({ error: 'Teléfono demasiado largo (máx 15 dígitos)' });
    }
    
    let codPais = codigoPais || '53';
    if (!/^\d{1,3}$/.test(codPais)) codPais = '53';
    
    // 3. Validar dirección si es a domicilio
    if (tipoServicio === 'domicilio' && ubicacion) {
        const palabra = contienePalabraProhibida(ubicacion);
        if (palabra) {
            const resultado = await registrarInfraccion(ip, 'palabra_prohibida_direccion', palabra);
            if (resultado.bloqueado) {
                return res.status(403).json({ 
            ...
            return res.status(400).json({ 
                error: `La dirección contiene la palabra prohibida: "${palabra}"`,
                advertencia: true,
                restantes: resultado.restantes
            });
        }
    ...
    // Validar que la fecha no sea pasada (si se proporciona fecha)
    if (fecha) {
        ...
        if (fechaObj < hoy) {
            ...
            if (proximaFecha) {
                return res.status(409).json({
                    ...
                    mensaje: `La fecha seleccionada ya pasó. Te sugiero ${proximaFecha.fechaTexto} a las ${proximaFecha.hora}:00 hs.`
                });
            }
            return res.status(400).json({ 
                error: 'La fecha seleccionada ya pasó',
                mensaje: `La fecha seleccionada ya pasó. Por favor, selecciona una fecha futura.`
            });
        }
    }
    ...
    if (ocupado) {
        const horariosDisponibles = HORAS_VALIDAS.filter(h => 
            !turnos.some(t => {
                if (t.fecha && fecha) {
                    return t.dia === diaLower && t.hora === h && t.fecha === fecha;
                }
                return t.dia === diaLower && t.hora === h;
            })
        );
...
console.log('📌 Validación de nombre, teléfono y dirección en servidor');
