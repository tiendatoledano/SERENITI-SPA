...
// server.js - verificar-codigo (CORREGIDO)
app.post('/api/registro/verificar-codigo', async (req, res) => {
    try {
        ...
        // ====== CREAR TOKEN ======
        const token = crypto.randomBytes(64).toString('hex');
        console.log(`🔑 Creando token para ${email}: ${token.substring(0, 20)}...`);
        ...
        try {
            ...
            // Si falla el token, el usuario ya está registrado, pero no puede iniciar sesión
            return res.status(500).json({ 
                error: 'Error al guardar la sesión. Por favor, intenta iniciar sesión manualmente.' 
            });
        }
...
// server.js - POST /turnos CORREGIDO con límite de 2 masajes por día
app.post('/turnos', async (req, res) => {
    try {
        ...
        // 2. ✅ NUEVO: Verificar límite de 2 masajes por día por usuario
        const telefonoLimpio = telefono.replace(/\D/g, '');
...
// server.js - Función WhatsApp CORREGIDA con imagen del servicio
app.post('/api/enviar-whatsapp/:id', verifyAuth, async (req, res) => {
    try {
        ...
        let imagenWhatsApp = 'https://i.postimg.cc/Hct3ps1K/photo-1519823551278-64ac92734fb1.jpg';
        
        try {
            // Obtener todos los servicios
            const servicios = await db.getServicios();
            console.log(`📋 Buscando imagen para: "${turno.massage_type}"`);
            
            // Buscar el servicio por nombre (case insensitive)
            ...
            if (servicio) {
                ...
                // Prioridad: imagen_whatsapp > imagen_web
                if (servicio.imagen_whatsapp && servicio.imagen_whatsapp.trim() !== '') {
                    imagenWhatsApp = servicio.imagen_whatsapp;
                    console.log(`📸 Usando imagen WhatsApp: ${imagenWhatsApp}`);
                } else if (servicio.imagen_web && servicio.imagen_web.trim() !== '') {
                    imagenWhatsApp = servicio.imagen_web;
                    console.log(`📸 Usando imagen Web: ${imagenWhatsApp}`);
                } else {
                    console.log('⚠️ Servicio sin imagen, usando imagen por defecto');
                }
            } else {
                console.log(`⚠️ Servicio "${turno.massage_type}" no encontrado, usando imagen por defecto`);
            }
        } catch (e) {
            console.error('❌ Error obteniendo imagen del servicio:', e);
        }
        
        // ====== CONVERTIR A URL DIRECTA DE POSTIMAGES ======
        // Si la URL es de postimg.cc, convertir a i.postimg.cc
        if (imagenWhatsApp.includes('postimg.cc') && !imagenWhatsApp.includes('i.postimg.cc')) {
            imagenWhatsApp = imagenWhatsApp.replace('postimg.cc', 'i.postimg.cc');
            // Si no tiene extensión, agregar .jpg
            if (!imagenWhatsApp.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
                imagenWhatsApp = imagenWhatsApp + '.jpg';
            }
        }
        
        console.log(`📸 Imagen final para WhatsApp: ${imagenWhatsApp}`);
...
*Equipo Serenity Spa*`;
        ...
        res.json({ 
            ...
            mensaje: mensaje,
            url: whatsappUrl,
            imagen: imagenWhatsApp  // ✅ Enviar la URL de la imagen
        });
...
// ============================================================
// HORARIOS - CORREGIDO CON MANEJO DE ERRORES
// ============================================================
app.get('/api/config/horarios', async (req, res) => {
    try {
        const horarios = await db.getHorarios();
        console.log('📋 GET /api/config/horarios ->', horarios);
        res.json(horarios);
    } catch (e) {
...
app.put('/api/config/horarios', verifyAuth, async (req, res) => {
    try {
        const data = req.body;
        console.log('📥 PUT /api/config/horarios - Datos recibidos:', JSON.stringify(data, null, 2));
        ...
        console.log('📤 Guardando en Supabase:', JSON.stringify(datosLimpios, null, 2));
        
        // Guardar en Supabase
        const result = await db.setHorarios(datosLimpios);
...
async function migrarDatos() {
    ...
    try {
        ...
        // Verificar y migrar testimonios
        const testimonios = await db.getTestimonios(false);
        ...
        // Verificar y cargar palabras baneadas
        const palabras = await db.getPalabrasBaneadas();
