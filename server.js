...
async function initAllFiles() {
    ...
    if (!fsSync.existsSync(CONFIG_FILE)) {
        ...
        try {
            ...
            console.warn('Error cargando config.json, usando valores por defecto');
        }
    }
...
// GET - Obtener configuración principal
app.get('/api/config', (req, res) => {
    res.json(configData);
});

// PUT - Guardar configuración principal
app.put('/api/config', async (req, res) => {
    ...
    try {
        ...
        configData = {
            hero: {
                titulo: data.hero?.titulo || configData.hero.titulo,
                subtitulo: data.hero?.subtitulo || configData.hero.subtitulo,
                imagenFondo: data.hero?.imagenFondo || configData.hero.imagenFondo,
                botonTexto: data.hero?.botonTexto || configData.hero.botonTexto
            },
            serviciosSection: {
                etiqueta: data.serviciosSection?.etiqueta || configData.serviciosSection.etiqueta,
                titulo: data.serviciosSection?.titulo || configData.serviciosSection.titulo,
                descripcion: data.serviciosSection?.descripcion || configData.serviciosSection.descripcion
            },
            contactoSection: {
                titulo: data.contactoSection?.titulo || configData.contactoSection.titulo,
                descripcion: data.contactoSection?.descripcion || configData.contactoSection.descripcion
            },
            shareSection: {
                titulo: data.shareSection?.titulo || configData.shareSection.titulo
            }
        };
...
async function getRegistroConfig() {
    try {
        ...
        console.warn('⚠️ Error en getRegistroConfig de Supabase:', e.message);
    }
...
async function saveRegistroConfig(data) {
    try {
        ...
        console.log('📋 Verificación de guardado - Beneficio 1:', verifyData.beneficios?.[0]?.descripcion);
...
app.get('/api/config/registro', async (req, res) => {
    try {
        ...
        console.log('📤 Enviando configuración de registro - Beneficio 1:', normalized.beneficios?.[0]?.descripcion);
        res.json(normalized);
    } catch (e) {
...
app.put('/api/config/registro', async (req, res) => {
    ...
    try {
        const data = req.body;
        console.log('📝 Recibiendo configuración de registro para guardar:');
...
// ============================================================
// SEGURIDAD - ESTADO ACTUAL (para el frontend)
// ============================================================
...
app.get('/api/seguridad/verificar-vpn', (req, res) => {
    // Simulación de verificación VPN
    // En un entorno real, aquí se verificaría la IP contra listas de VPN/Proxy
    const ip = req.ip || req.socket.remoteAddress || '0.0.0.0';
    const ipLimpia = ip.replace('::ffff:', '');
    ...
    // Verificar si es una IP sospechosa (simulado)
    // En producción, se usaría una API como ip-api.com o similar
...
app.post('/api/seguridad/infraccion', (req, res) => {
    ...
    // Si tiene 3 infracciones, bloquear
...
app.post('/api/premium/usuario', async (req, res) => {
    try {
        // Por ahora solo respondemos éxito ya que no tenemos implementación de BD
        console.log('📝 Registro premium solicitado:', req.body);
        res.json({ success: true, mensaje: 'Usuario registrado correctamente' });
    } catch (e) {
        console.error('Error registrando usuario premium:', e);
