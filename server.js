...
// ============================================================
// ⭐ FUNCIÓN PARA ENVIAR EMAIL CON EMAILJS (NUEVA)
// ============================================================
async function enviarEmail(email, nombre, codigo) {
    try {
        // Usar fetch para llamar a la API de EmailJS desde el servidor
        const publicKey = process.env.EMAILJS_PUBLIC_KEY;
        const serviceId = process.env.EMAILJS_SERVICE_ID;
        const templateId = process.env.EMAILJS_TEMPLATE_ID;
        
        if (!publicKey || !serviceId || !templateId) {
            console.error('❌ EmailJS no configurado correctamente');
        ...
        const response = await fetch(url, {
            ...
            headers: {
                ...
                'Origin': 'https://serenity-spa-2utz.onrender.com'
            },
...
app.post('/api/registro/guardar-codigo', async (req, res) => {
    try {
        ...
        const resultadoEmail = await enviarEmail(email, nombre, codigo);
        
        if (!resultadoEmail.success) {
            console.warn(`⚠️ No se pudo enviar email a ${email}: ${resultadoEmail.error}`);
            // No fallamos el registro, solo advertimos
        }
...
// ============================================================
// ⭐ CORREGIDO: LOGIN - SOLICITAR CÓDIGO CON ENVÍO DE EMAIL
// ============================================================
app.post('/api/registro/login-solicitar-codigo', async (req, res) => {
    try {
        ...
        // Si el usuario está bloqueado → error
        ...
        // Si el usuario no está verificado → error
        ...
        // ⭐⭐ ENVIAR EMAIL CON EL CÓDIGO - ESTA ES LA PARTE QUE FALTABA
        const resultadoEmail = await enviarEmail(email, usuarioExistente.nombre, codigo);
        
        if (!resultadoEmail.success) {
            console.error(`❌ Error enviando email a ${email}: ${resultadoEmail.error}`);
            return res.status(500).json({ 
                error: 'Error al enviar el código al correo. Verifica tu conexión o contacta al administrador.' 
            });
        }
