...
app.get('/api/turnos/mis-turnos', async (req, res) => {
    ...
    try {
        ...
        console.log('🔍 Buscando turnos para usuario:', usuario.nombre, 'telefono:', usuario.telefono);
        
        // Buscar turnos por teléfono o nombre
        let query = supabase
            .from('turnos')
            .select('*')
            .order('fecha_creacion', { ascending: false });
        
        // Si el usuario tiene teléfono, buscar por teléfono
        if (usuario.telefono) {
...
// ============================================================
app.post('/api/turnos/verificar-automatico', async (req, res) => {
    try {
        const ahora = new Date();
...
app.get('/api/reservas/mias', async (req, res) => {
    ...
    try {
        ...
        if (usuario.telefono) {
            ...
            query = query.or(`nombre.eq.${usuario.nombre},telefono.eq.${usuario.telefono || ''}`);
        }
