...
// ============================================================
// CONFIGURACIÓN REGISTRO - CORREGIDO CON IMAGEN
// ============================================================
...
app.put('/api/config/registro', async (req, res) => {
    ...
    try {
        ...
        // Validar que los arrays tengan la longitud correcta
        if (data.beneficios.length !== 6) {
            return res.status(400).json({ 
                success: false, 
                error: 'Deben haber exactamente 6 beneficios' 
            });
        }
        
        if (data.conceptos.length !== 6) {
            return res.status(400).json({ 
                success: false, 
                error: 'Deben haber exactamente 6 conceptos' 
            });
        }
        
        // IMPORTANTE: Asegurar que la imagen de WhatsApp se guarde
...
app.post('/api/enviar-whatsapp/:id', async (req, res) => {
    ...
    try {
        ...
        const mensaje = `🌿 *SERENITY SPA*\n\n` +
            ...
            `💆 *Masaje:* ${turno.massageType || 'Masaje'}\n` +
