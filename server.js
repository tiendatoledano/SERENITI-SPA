...
try {
    ...
    if (SUPABASE_URL && SUPABASE_ANON_KEY) {
        ...
        logWarn('Supabase no configurado. Usando almacenamiento local.');
    }
} catch (e) {
    logWarn('Módulo @supabase/supabase-js no encontrado. Usando almacenamiento local.', e);
}
...
app.use((req, res, next) => {
    ...
    // Capturar la respuesta para loguear el status
...
// ============================================================
// CONFIGURACIÓN REGISTRO - CON LOGS EXTENSIVOS
// ============================================================
...
async function getRegistroConfig() {
    ...
    try {
        if (supabase) {
            ...
            if (!error && data) {
                ...
                logInfo('📋 Datos de Supabase:', data);
                return data;
            } else if (error) {
    ...
    try {
        if (fsSync.existsSync(REGISTRO_FILE)) {
            ...
            logInfo('📋 Datos del archivo local:', data);
            return data;
        } else {
...
async function saveRegistroConfig(data) {
    ...
    logInfo('📋 Datos a guardar:', data);
    
    try {
        const normalizedData = normalizeRegistroConfig(data);
        logInfo('📋 Datos normalizados:', normalizedData);
        ...
        try {
            ...
            logInfo('📋 Beneficio 1 guardado:', verifyData.beneficios?.[0]?.descripcion);
        } catch (verifyError) {
        ...
        if (supabase) {
            try {
                ...
                if (result.error) {
                    logWarn('⚠️ Error guardando en Supabase:', result.error.message);
                } else {
                    logSuccess('✅ Configuración guardada en Supabase');
                }
            } catch (e) {
                logWarn('⚠️ Error en operación de Supabase:', e.message);
            }
        }
...
// ============================================================
// RUTAS DE REGISTRO - CON LOGS EXTENSIVOS
// ============================================================
...
app.put('/api/config/registro', async (req, res) => {
    ...
    try {
        const data = req.body;
        logInfo('📝 PUT /api/config/registro - Datos recibidos:', data);
        ...
        if (!data.beneficios || !Array.isArray(data.beneficios)) {
            logWarn('⚠️ No se recibieron beneficios, usando valores por defecto');
            data.beneficios = [
                { descripcion: 'Porcentaje de descuento en todos los servicios (configurable por admin)' },
                { descripcion: 'Reserva paquetes de 5, 10, o 15 días consecutivos con precio especial' },
                { descripcion: 'Cada año de membresía activa, recibe un masaje completamente gratis' },
        ...
        if (!data.conceptos || !Array.isArray(data.conceptos)) {
            logWarn('⚠️ No se recibieron conceptos, usando valores por defecto');
            data.conceptos = [
                { titulo: 'Masaje Sueco', descripcion: 'Movimientos largos y fluidos para relajar los músculos superficiales.' },
                { titulo: 'Tejido Profundo', descripcion: 'Trabaja en capas musculares profundas. Ideal para contracturas crónicas.' },
                { titulo: 'Masaje Deportivo', descripcion: 'Pre-competencia, post-competencia y mantenimiento para atletas.' },
...
app.get('/api/seguridad/verificar-vpn', (req, res) => {
    ...
    // Simulación básica de detección de VPN
    // En producción, usar una API real como ip-api.com
