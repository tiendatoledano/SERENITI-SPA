...
function validarTelefonoCompleto(telefono) {
    ...
    // ❌ NO permitir números repetidos (ej: 2222222, 0000000)
    if (/^(\d)\1+$/.test(soloNumeros)) {
        return { valido: false, motivo: 'El teléfono no puede ser una secuencia repetida' };
    }
    // ❌ NO permitir secuencias comunes (ej: 1234567, 0123456, 8765432)
    ...
    // ❌ NO permitir patrones como 0000909, 30303030, 222333909
    if (/^0{3,}[0-9]/.test(soloNumeros) && !/^0{3,}[1-9]/.test(soloNumeros)) {
        return { valido: false, motivo: 'El teléfono contiene un patrón no válido' };
    }
    // Verificar que no sea un patrón repetitivo como 30303030, 12121212, 222333909
    ...
    // Verificar que no sea un patrón como 222333909 (repetición de grupos)
...
async function registrarInfraccion(ip, tipo, detalle) {
    ...
    if (data.count >= 3) {
        const motivo = `3 infracciones en 24 horas: ${tipo} (${detalle})`;
        bloquearIP(ipLimpia, motivo, 'INFRACCIONES_MULTIPLES', detalle, false);
...
console.log('✅ Servidor completamente blindado con sistema de infracciones mejorado');
console.log('📌 Las variables HORAS_VALIDAS y DIAS_VALIDOS se actualizan dinámicamente');
console.log('📌 Sistema de 3 infracciones = bloqueo automático de IP');
...
console.log('📌 Validación de dirección: detecta palabras prohibidas configuradas en admin');
console.log('📌 Calendario: verifica fechas específicas para disponibilidad');
console.log('📌 WhatsApp: incluye dirección del salón y código de confirmación');
console.log('📌 Ubicación del salón configurable desde admin');
console.log('📌 Configuración de registro: actualizable desde admin en tiempo real');