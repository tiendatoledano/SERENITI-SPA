...
// ============================================================
// REGISTRO DE USUARIOS (resumido - mantener igual)
// ============================================================
...
app.get('/api/turnos/mis-turnos', async (req, res) => {
    ...
    try {
        ...
        // Intentar agregar imagen del servicio
        for (const turno of turnos) {
            try {
                const { data: servicioData } = await supabase
                    .from('servicios')
                    .select('imagen_web')
                    .ilike('nombre', `%${turno.servicio}%`)
                    .maybeSingle();
                
                if (servicioData && servicioData.imagen_web) {
                    turno.imagen = servicioData.imagen_web;
                }
            } catch (e) {}
        }
