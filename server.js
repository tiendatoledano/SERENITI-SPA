...
// ============================================================
// RUTA DE BLOQUEO - DEBE IR ANTES DE CUALQUIER OTRA RUTA
// ============================================================
...
// ============================================================
// ⭐ MIDDLEWARE DE BLOQUEO DE IP - SE EJECUTA ANTES DE CUALQUIER RUTA
// ============================================================
...
app.get('/terminos.html', (req, res) => res.sendFile(path.join(BASE_DIR, 'terminos.html')));
...
app.post('/api/reservas/cancelar', async (req, res) => {
    ...
    const token = authHeader.substring(7);
    const { id, codigoCancelacion } = req.body;
    
    if (!id || !codigoCancelacion) {
        return res.status(400).json({ error: 'Datos incompletos' });
    }
    ...
    try {
        ...
        const { error } = await supabase
            .from('reservas')
            .update({ estado: 'cancelacion_solicitada' })
            .eq('id', id)
            .eq('usuario_id', usuario.id);
        ...
        console.error('Error en /api/reservas/cancelar:', e);
        res.status(500).json({ error: 'Error interno' });
    }
});

app.post('/api/reservas/rectificar', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'No autorizado' });
    }
    ...
    try {
        ...
        if (error) {
            console.error('Error rectificando reserva:', error);
            return res.status(500).json({ error: 'Error al rectificar reserva' });
        }
        
        res.json({ success: true });
    } catch (e) {
        console.error('Error en /api/reservas/rectificar:', e);
        res.status(500).json({ error: 'Error interno' });
    }
});
