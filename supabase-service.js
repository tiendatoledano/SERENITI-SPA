...
// ✅ Exportar supabase para usarlo en server.js
...
async function updateTurno(id, data) {
    try {
        ...
        if (data.confirmado_whatsapp !== undefined) {
            updateData.confirmado_whatsapp = data.confirmado_whatsapp;
        }
        if (data.conteo_bloqueos !== undefined) {
            updateData.conteo_bloqueos = data.conteo_bloqueos;
        }
...
async function getBloqueosByIP(ip) {
    try {
        const { data, error } = await supabase
            .from('bloqueos')
            .select('*')
        ...
        console.error('❌ Error en getBloqueosByIP:', error);
        return [];
    }
}
