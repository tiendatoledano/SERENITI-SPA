...
// ⭐ CREAR CLIENTE ADMIN CON CONFIGURACIÓN CORRECTA
...
async function getUsuarioByToken(token) {
    try {
        ...
        if (tokenError || !tokenData) {
            console.log('⚠️ Token no encontrado o expirado');
            return null;
        }
        ...
        if (userError || !usuario) {
            console.log('⚠️ Usuario no encontrado para el token');
            return null;
        }
...
async function createTokenUsuario(token, email) {
    try {
        ...
        if (userError || !usuario) {
            console.error('❌ Usuario no encontrado para crear token:', email);
...
async function deleteTokenUsuario(token) {
    try {
        ...
        console.error('Error en deleteTokenUsuario:', e);
        throw e;
    }
}
...
async function getMagicLinkByToken(token) {
    try {
        ...
        console.error('Error en getMagicLinkByToken:', e);
        return null;
    }
}

async function createMagicLink(email, token, expiracion, nombre) {
    try {
        ...
        console.error('Error en createMagicLink:', e);
        throw e;
    }
}
...
async function markMagicLinkAsUsed(token) {
    try {
        ...
        console.error('Error en markMagicLinkAsUsed:', e);
        throw e;
    }
}

async function deleteOldMagicLinks() {
    try {
        ...
        if (error) {
            console.error('Error deleting old magic links:', error);
        }
        return true;
    } catch (e) {
        console.error('Error en deleteOldMagicLinks:', e);
        return false;
    }
}

// ============================================================
// ⭐ INTENTOS DE LOGIN (Anti-Abuso)
// ============================================================

async function registrarIntentoLogin(email, ip, tipo) {
    try {
        const { error } = await supabase
            .from('intentos_login')
            .insert({
        ...
        console.error('Error en registrarIntentoLogin:', e);
        return false;
    }
}

async function getIntentosLoginRecientes(email, ip, minutos = 15) {
    try {
        ...
        console.error('Error en getIntentosLoginRecientes:', e);
        return 0;
    }
}
...
async function limpiarCodigosExpirados() {
    try {
        ...
        console.error('Error en limpiarCodigosExpirados:', e);
        return false;
    }
}
...
module.exports = {
    ...
    // ⭐ NUEVAS FUNCIONES - Magic Links
    getMagicLinkByToken,
    createMagicLink,
    markMagicLinkAsUsed,
    deleteOldMagicLinks,
    
    // ⭐ NUEVAS FUNCIONES - Anti-Abuso
    registrarIntentoLogin,
    getIntentosLoginRecientes,
