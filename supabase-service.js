    ...
    try {
        ...
        if (error) {
            if (error.code === 'PGRST116') {
                return {
                    hero: { titulo: 'Comienza tu viaje de bienestar', subtitulo: 'Regístrate y descubre el poder del masaje', boton: 'Unirme ahora' },
                    beneficios: [
...
async function createTokenUsuario(token, email) {
    try {
        if (!token || !email) throw new Error('Token y email son requeridos');
        const { error } = await supabase
            .from('tokens_usuarios')
            .insert({ token: token, email: email, created_at: new Date().toISOString() });
        if (error) throw error;
        return { success: true };
    } catch (e) {
...
async function upsertCodigoVerificacion(email, data) {
    try {
        if (!email || !data) throw new Error('Email y datos son requeridos');
        const { error } = await supabase
            .from('codigos_verificacion')
            .upsert({ 
                email: email,
